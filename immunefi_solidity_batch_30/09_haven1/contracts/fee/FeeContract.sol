    ...
    IVersion
{
    ...
    function initialize(
        ...
        if (assocShare_ > 1e18) {
            revert FeeContract__InvalidAssociationShare();
        }
    ...
    function adjustChannel(
        ...
        uint256 shares = _contractShares;

        shares -= _weights[index];
        shares += weight_;

        _weights[index] = weight_;
        _channels[index] = newChannel_;

        _contractShares = shares;
    ...
    function updateFee() external {
        ...
        bool success = _refreshOracle();

        // We return early here if the refresh was not successful rather than
        // reverting the transaction as there are other services and contracts
        // that rely on calls to this (updateFee) function to not fail. Returning
        // early simply means that the previous fee will remain active.
        if (!success) return;
     ...
     * @inheritdoc IFeeContract
     */
    function setMinFee(uint256 fee_) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     * @inheritdoc IFeeContract
     */
    function setMaxFee(uint256 fee_) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     * @inheritdoc IFeeContract
     */
    function setOracle(address addr_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        addr_.assertNotZero();
    ...
    function setExemptCaller(
        ...
        contract_.assertNotZero();
        caller_.assertNotZero();
    ...
    function setExemptContract(
        ...
        if (!isContract) {
            revert FeeContract__OnlyContract(contract_);
        }
     ...
     * @inheritdoc IFeeContract
     */
    function setFeeUSD(uint256 feeUSD_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _feeUSD = feeUSD_;
        emit FeeUSDSet(feeUSD_);
    }
    ...
    function setAssocShare(
        ...
        _assocShare = assocShare_;
        emit AssociationShareSet(assocShare_);
    }
    ...
    function _distributeFees() private {
        ...
        uint256 n = _channels.length;
        uint256 shares = _contractShares;

        for (uint8 i; i < n; ++i) {
            uint256 share = (amount * _weights[i]) / shares;
