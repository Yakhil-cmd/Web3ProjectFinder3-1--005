    ...
    IVersion
{
     ...
     * @param maxIters_     The maximum iteration count.
     *
     * @dev Requirements:
    ...
    function setAssociation(
        ...
        _revokeRole(NETWORK_GUARDIAN, prev);
     ...
     * @inheritdoc INetworkGuardianController
     */
    function setMaxIters(uint256 n) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _assertValidIters(n);

        uint256 prev = _maxIters;
        _maxIters = n;

        emit MaxItersUpdated(prev, n);
    }
    ...
    function getRanges() external view returns (Range[] memory) {
        return Array.ranges(_maxIters, _registeredAddresses.length);
    }
    ...
    function _actionMultiple(
        ...
        if (len > _maxIters) {
            revert NetworkGuardianController__MaxIterations(len, _maxIters);
        }
    ...
    function _actionRange(
        ...
        uint256 d = end - start;
        if (d > _maxIters) {
            revert NetworkGuardianController__MaxIterations(d, _maxIters);
        }
    ...
    /**
     * @notice Asserts that a given iteration count is greater than zero.
     *
     * @param n The number to check.
     */
    function _assertValidIters(uint256 n) private pure {
        if (n == 0) revert NetworkGuardianController__InvalidIterationCount();
    }
}
