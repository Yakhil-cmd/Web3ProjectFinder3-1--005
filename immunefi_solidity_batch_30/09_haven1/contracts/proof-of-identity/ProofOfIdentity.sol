    ...
    IVersion
{
    ...
    function isSuspended(address account) external view returns (bool) {
        return _accountManager.getAccountStatus(account) == 4;
    }
    ...
    function setAttributeName(
        uint256 id,
        string calldata name
    ) public onlyRole(DEFAULT_ADMIN_ROLE) whenNotGuardianPaused {
        if (id >= _attributeCount) {
            revert ProofOfIdentity__InvalidAttribute(id);
        }
        ...
        emit AttributeNameUpdated(id, name);
    }
    ...
    function setAttributeType(
        ...
        emit AttributeTypeUpdated(id, attrType);
    }
    ...
    function _assertIsNotSuspended(address addr) private view {
        if (_accountManager.getAccountStatus(addr) == 4) {
