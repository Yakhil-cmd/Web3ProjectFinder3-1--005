...
abstract contract ContextHelper is ReadManifestHelper {
    ...
    uint256 public constant BASE_RATE_SLOT = 104;
    uint256 public constant PRICE_FEED_SLOT = 2;
    ...
    uint256 public constant UPDATED_AT_TIME_BUFFER = 10_000 seconds;
    uint256 public constant DECIMAL_PRECISION = 1e18;

    uint256 public constant MAX_UINT256 = type(uint256).max;
    uint256 public constant STANDARD_HINT = 0;
    uint256 public constant REDUCED_BASE_RATE = 1e16;

    bool public constant SKIP_TEST = true;
    address public constant THUNDERHEAD_OVERSEER = 0xB96f07367e69e86d6e9C3F29215885104813eeAE;
    ...
    function setUp() public virtual override skipTest {
        super.setUp();
    }
    ...
    function _getPriceFeedFromSlot(address _contract) internal view returns (address) {
        return address(uint160(uint256(vm.load(_contract, bytes32(PRICE_FEED_SLOT)))));
    }
    ...
    function _dealAllTokens(address _to, uint256 _hypeAmount, uint256 _collateralAmount) internal {
        ...
        for (uint8 i = 0; i < BRANCHES_LENGTH; i++) {
            if(Collaterals(i) == Collaterals.WSTHYPE) {
                continue;
            } else {
...
/*             address _collateralAddress = _getCollateralAddress(Collaterals(i));
    ...
    function _dealWSTHYPE(address _to, uint256 _amount) internal {
        vm.startPrank(_to);
        IOverseer(THUNDERHEAD_OVERSEER).mint{value: _amount}(_to);
        vm.stopPrank();
    }
    ...
    function _lowerBaseRate() internal {
        /// @notice This is done to make redemptions realistic and not make pass time after a collateral is added otherwise 
        /// otherwise we would need to touch the oracle updated time which is a longer process
        vm.store(address(singletonContracts.collateralRegistry), bytes32(BASE_RATE_SLOT), bytes32(REDUCED_BASE_RATE));
    }
}