...
contract InvariantsTest is Assertions, Logging, BaseInvariantTest, BaseMultiCollateralTest {
    ...
    function setUp() public override {
        ...
        if (n > 0) {
            p[0] = TestDeployer.TroveManagerParams(1.5 ether, 1.1 ether, 1.1 ether, 0.05 ether, 0.1 ether, type(uint256).max);
        }
        if (n > 1) {
            p[1] = TestDeployer.TroveManagerParams(1.6 ether, 1.2 ether, 1.2 ether, 0.05 ether, 0.2 ether, type(uint256).max);
        }
        if (n > 2) {
            p[2] = TestDeployer.TroveManagerParams(1.6 ether, 1.2 ether, 1.2 ether, 0.05 ether, 0.2 ether, type(uint256).max);
        }
        if (n > 3) {
            p[3] = TestDeployer.TroveManagerParams(1.6 ether, 1.25 ether, 1.01 ether, 0.05 ether, 0.1 ether, type(uint256).max);
        }

        TestDeployer deployer = new TestDeployer();
        Contracts memory contracts;
        (contracts.branches, contracts.collateralRegistry, contracts.feUSDToken, contracts.hintHelpers,, contracts.whype,)
    ...
    function invariant_FundsAreSwept() external view {
        for (uint256 i = 0; i < actors.length; ++i) {
            ...
            assertEqDecimal(feUSDToken.balanceOf(actor), 0, 18, "Incomplete BOLD sweep");
            assertEqDecimal(whype.balanceOf(actor), 0, 18, "Incomplete WETH sweep");
            ...
            for (uint256 j = 0; j < branches.length; ++j) {
                ...
                assertEqDecimal(whype.allowance(actor, borrowerOperations), 0, 18, "WETH allowance != 0");
    ...
    function invariant_SystemStateMatchesGhostState() external view {
        for (uint256 i = 0; i < branches.length; ++i) {
            ...
            assertEqDecimal(whype.balanceOf(address(c.pools.gasPool)), handler.getGasPool(i), 18, "Wrong GasPool");
            assertApproxEqAbsDecimal(
                c.pools.collSurplusPool.getCollBalance(), handler.collSurplus(i), 10, 18, "Wrong CollSurplusPool"
            );
            assertApproxEqAbsDecimal(
                c.stabilityPool.getTotalfeUSDDeposits(),
                handler.spfeUSDDeposits(i),
                100,
                18,
    ...
    function invariant_AllfeUSDBackedByTroveDebt() external view {
        uint256 totalfeUSD = feUSDToken.totalSupply();
        ...
        assertApproxEqAbsDecimal(
            totalfeUSD + totalPendingInterest + totalPendingBatchManagementFees,
            totalDebt,
            1e-10 ether,
            18,
            "Total feUSD !~= total debt"
        );
    }
    ...
    function invariant_StabilityPool_AllfeUSDClaimable_ExceptYieldReceivedWhileEmpty() external view {
        for (uint256 j = 0; j < branches.length; ++j) {
            IStabilityPool stabilityPool = branches[j].stabilityPool;

            uint256 sumfeUSDDeposit = 0;
            uint256 sumYieldGain = 0;
            uint256 yieldGainsPending = stabilityPool.getYieldGainsPending();

            for (uint256 i = 0; i < actors.length; ++i) {
                sumfeUSDDeposit += stabilityPool.getCompoundedfeUSDDeposit(actors[i].account);
                sumYieldGain += stabilityPool.getDepositorYieldGain(actors[i].account);
            }

            assertLt(
                stabilityPool.getTotalfeUSDDeposits() - sumfeUSDDeposit, 1000, "totalfeUSDDeposits !~= sum(boldDeposit)"
            );
            ...
            assertLt(
                feUSDToken.balanceOf(address(stabilityPool)) - sumfeUSDDeposit - sumYieldGain - yieldGainsPending,
                1000,
