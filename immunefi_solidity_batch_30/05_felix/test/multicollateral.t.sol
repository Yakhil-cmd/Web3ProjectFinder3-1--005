...
contract MulticollateralTest is DevTestSetup {
    ...
    function setUp() public override {
        ...
        troveManagerParamsArray[0] = TestDeployer.TroveManagerParams(150e16, 110e16, 110e16, 5e16, 10e16, 10_000_000 ether);
        troveManagerParamsArray[1] = TestDeployer.TroveManagerParams(160e16, 120e16, 120e16, 5e16, 10e16, 10_000_000 ether);
        troveManagerParamsArray[2] = TestDeployer.TroveManagerParams(160e16, 120e16, 120e16, 5e16, 10e16, 10_000_000 ether);
        troveManagerParamsArray[3] = TestDeployer.TroveManagerParams(160e16, 125e16, 125e16, 5e16, 10e16, 10_000_000 ether);

        TestDeployer deployer = new TestDeployer();
        TestDeployer.LiquityContractsDev[] memory _contractsArray;
        (_contractsArray, collateralRegistry, feUSDToken,,, WHYPE,) =
        ...
        for (uint256 c = 0; c < NUM_COLLATERALS; c++) {
            for (uint256 i = 0; i < 6; i++) {
                ...
                WHYPE.approve(address(contractsArray[c].borrowerOperations), type(uint256).max);
                vm.stopPrank();
            }
        }
    }
    // Now it revets directly when you call an out of bound index
    ...
    function testMultiCollateralRedemption() public {
        ...
        // Third collateral unbacked feUSD: 1k
        ...
        // Fourth collateral unbacked feUSD: 0
        ...
        assertEq(feUSDToken.balanceOf(A), 16000e18, "Wrong feUSD balance before redemption");
        ...
        // fees
        uint256 fee = collateralRegistry.getEffectiveRedemptionFeeInfeUSD(redeemAmount);
        ...
        // Check feUSD balance
        // TODO: change when we switch to new gas compensation
        assertApproxEqAbs(feUSDToken.balanceOf(A), 14400e18, 10, "Wrong feUSD balance after redemption");
    ...
    function testMultiCollateralRedemptionFuzz(
        uint256 _spfeUSDAmount1,
        uint256 _spfeUSDAmount2,
        uint256 _spfeUSDAmount3,
        uint256 _spfeUSDAmount4,
        uint256 _redemptionFraction
    ) public {
        uint256 feUSDAmount = 10000e18;
        uint256 minfeUSDBalance = 1;
        // TODO: remove gas compensation
        _spfeUSDAmount1 = bound(_spfeUSDAmount1, 0, feUSDAmount);
        _spfeUSDAmount2 = bound(_spfeUSDAmount2, 0, feUSDAmount);
        _spfeUSDAmount3 = bound(_spfeUSDAmount3, 0, feUSDAmount);
        _spfeUSDAmount4 = bound(_spfeUSDAmount4, 0, feUSDAmount - minfeUSDBalance);
        _redemptionFraction = bound(_redemptionFraction, DECIMAL_PRECISION / minfeUSDBalance, DECIMAL_PRECISION);

        _testMultiCollateralRedemption(
            feUSDAmount, _spfeUSDAmount1, _spfeUSDAmount2, _spfeUSDAmount3, _spfeUSDAmount4, _redemptionFraction
        );
    }
    ...
    function testMultiCollateralRedemptionMaxSPAmount() public {
        ...
        _testMultiCollateralRedemption(
            ...
            feUSDAmount,
            feUSDAmount,
            feUSDAmount - minfeUSDBalance,
            DECIMAL_PRECISION / minfeUSDBalance
        );
    }
    ...
    function _testMultiCollateralRedemption(
        ...
        // First collateral
        openMulticollateralTroveNoHints100pctWithIndex(0, A, 0, 10e18, _feUSDAmount, 5e16);
        if (_spfeUSDAmount1 > 0) makeMulticollateralSPDepositAndClaim(0, A, _spfeUSDAmount1);

        // Second collateral
        testValues2.troveId = openMulticollateralTroveNoHints100pctWithIndex(1, A, 0, 100e18, _feUSDAmount, 5e16);
        if (_spfeUSDAmount2 > 0) makeMulticollateralSPDepositAndClaim(1, A, _spfeUSDAmount2);

        // Third collateral
        openMulticollateralTroveNoHints100pctWithIndex(2, A, 0, 10e18, _feUSDAmount, 5e16);
        if (_spfeUSDAmount3 > 0) makeMulticollateralSPDepositAndClaim(2, A, _spfeUSDAmount3);

        // Fourth collateral
        openMulticollateralTroveNoHints100pctWithIndex(3, A, 0, 10e18, _feUSDAmount, 5e16);
        if (_spfeUSDAmount4 > 0) makeMulticollateralSPDepositAndClaim(3, A, _spfeUSDAmount4);

        uint256 feUSDBalance = feUSDToken.balanceOf(A);
        // Check A’s final bal
        // TODO: change when we switch to new gas compensation
        //assertEq(feUSDToken.balanceOf(A), _feUSDAmount * 4 - _spfeUSDAmount1 - _spfeUSDAmount2 - _spfeUSDAmount3 - _spfeUSDAmount4, "Wrong feUSD balance before redemption");
        // Stack too deep
        //assertEq(feUSDBalance, _feUSDAmount * 4 - _spfeUSDAmount1 - _spfeUSDAmount2 - _spfeUSDAmount3 - _spfeUSDAmount4, "Wrong feUSD balance before redemption");

        uint256 redeemAmount = feUSDBalance * _redemptionFraction / DECIMAL_PRECISION;
        ...
        testValues1.unbackedPortion = contractsArray[0].troveManager.getEntireBranchDebt() - _spfeUSDAmount1;
        testValues2.unbackedPortion = contractsArray[1].troveManager.getEntireBranchDebt() - _spfeUSDAmount2;
        testValues3.unbackedPortion = contractsArray[2].troveManager.getEntireBranchDebt() - _spfeUSDAmount3;
        testValues4.unbackedPortion = contractsArray[3].troveManager.getEntireBranchDebt() - _spfeUSDAmount4;
        uint256 totalUnbacked = testValues1.unbackedPortion + testValues2.unbackedPortion + testValues3.unbackedPortion
            + testValues4.unbackedPortion;
        ...
        // Check feUSD balance
        assertApproxEqAbs(feUSDToken.balanceOf(A), feUSDBalance - redeemAmount, 10, "Wrong feUSD balance after redemption");
    ...
    function testMultiCollRedemptionIncreasesRedeemerETHBalanceByCorrespondingETHLessTheETHFee() public {
        ...
        uint256 feUSDAmount = 100000e18;
        testValues0.spfeUSDAmount = feUSDAmount / 2;
        testValues1.spfeUSDAmount = feUSDAmount / 4;
        testValues2.spfeUSDAmount = feUSDAmount / 8;
        testValues3.spfeUSDAmount = feUSDAmount / 16;
        ...
        // First collateral
        openMulticollateralTroveNoHints100pctWithIndex(0, A, 0, 100e18, feUSDAmount, 5e16);
        makeMulticollateralSPDepositAndClaim(0, A, testValues0.spfeUSDAmount);

        // Second collateral
        openMulticollateralTroveNoHints100pctWithIndex(1, A, 0, 1000e18, feUSDAmount, 5e16);
        makeMulticollateralSPDepositAndClaim(1, A, testValues1.spfeUSDAmount);

        // Third collateral
        openMulticollateralTroveNoHints100pctWithIndex(2, A, 0, 100e18, feUSDAmount, 5e16);
        makeMulticollateralSPDepositAndClaim(2, A, testValues2.spfeUSDAmount);

        // Fourth collateral
        openMulticollateralTroveNoHints100pctWithIndex(3, A, 0, 100e18, feUSDAmount, 5e16);
        makeMulticollateralSPDepositAndClaim(3, A, testValues3.spfeUSDAmount);

        uint256 feUSDBalance = feUSDToken.balanceOf(A);

        uint256 redeemAmount = feUSDBalance * redemptionFraction / DECIMAL_PRECISION;
        uint256 expectedFeePct =
            collateralRegistry.getEffectiveRedemptionFeeInfeUSD(redeemAmount) * DECIMAL_PRECISION / redeemAmount;
        assertGt(expectedFeePct, 0);

        // Get feUSD debts from each branch
        ...
        // Check how much feUSD was redeemed from each branch
    ...
    function testMultiCollateralRedemptionWithZeroUnbacked() public {
        ...
        // First collateral unbacked feUSD: 10k (SP empty) - will be shutdown
        testValues1.troveId = openMulticollateralTroveNoHints100pctWithIndex(0, A, 0, 10e18, 10000e18, 5e16);

        // Second collateral unbacked feUSD: 0
        testValues2.troveId = openMulticollateralTroveNoHints100pctWithIndex(1, A, 0, 100e18, 10000e18, 5e16);
        makeMulticollateralSPDepositAndClaim(1, A, 10100e18); // we put some more for interest

        // Third collateral unbacked feUSD: 0
        ...
        assertEq(feUSDToken.balanceOf(A), 9700e18, "Wrong feUSD balance before redemption");
        ...
        // Check feUSD balance
        assertApproxEqAbs(feUSDToken.balanceOf(A), 8100e18, 10, "Wrong feUSD balance after redemption");
    ...
    function testMultiCollateralRedemptionWithZeroUnbackedLowSCRButNoShutdown() public {
        ...
        // First collateral unbacked feUSD: 10k (SP empty) - will become below SCR
        testValues1.troveId = openMulticollateralTroveNoHints100pctWithIndex(0, A, 0, 10e18, 10000e18, 5e16);
