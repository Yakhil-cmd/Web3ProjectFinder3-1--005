...
contract RebasingBatchShares is DevTestSetup {
    ...
    // See: https://github.com/GalloDaSballo/feUSD-review/issues/42
    function testBatchRebaseToSystemInsolvency() public {
        ...
        vm.startPrank(C);
        feUSDToken.transfer(A, feUSDToken.balanceOf(C));
        vm.stopPrank();
     ...
     // === 4: Free Loans === //
        ...
        uint256 balB4 = feUSDToken.balanceOf(A);
        closeTrove(A, anotherATroveId);
        uint256 balAfter = feUSDToken.balanceOf(A);
    ...
    function _addDebtAndEnsureItMintsShares(uint256 troveId, address caller, uint256 amt) internal {
        ...
        withdrawfeUSD100pct(caller, troveId, amt);
    ...
    function testDeflateDebtLeavingSharesConstant() public {
        ...
        vm.startPrank(B);
        while (x++ < ITERATIONS) {
            borrowerOperations.repayfeUSD(BTroveId, 1);
        }
        vm.stopPrank();
