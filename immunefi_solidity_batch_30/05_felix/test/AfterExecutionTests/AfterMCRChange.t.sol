...
contract AfterMCRChange is ContextHelper {
    ...
    uint256 public constant NEW_MCR = 1428570000000000000;

    uint256 public constant TROVE_MANAGER_MCR_SLOT = 63;

    uint256 public constant MIN_DEBT = 2_000 ether;
    ...
    function test_newMCR_after_apply() public {
        IAddressesRegistry _addressRegistry = IAddressesRegistry(branchContracts[Collaterals.WBTC].addressesRegistry);
        IBorrowerOperations _borrowerOperations = IBorrowerOperations(branchContracts[Collaterals.WBTC].borrowerOperations);
        address _troveManager = branchContracts[Collaterals.WBTC].troveManager;
        assertEq(_addressRegistry.MCR(), NEW_MCR, "MCR not updated in address registry");
        assertEq(_borrowerOperations.MCR(), NEW_MCR, "MCR not updated in borrower operations");
        assertEq(_getMCRInTroveManager(_troveManager), NEW_MCR, "MCR not updated in trove manager");
    }
    ...
    function _applyNewMCR() internal {
        vm.startPrank(multiSig);
        IAdminController(singletonContracts.adminController).applyMCR(NEW_BRANCH_INDEX);
        vm.stopPrank();
    }
    ...
    function _getMCRInTroveManager(address _troveManager) internal view returns (uint256) {
        return uint256(vm.load(address(_troveManager), bytes32(TROVE_MANAGER_MCR_SLOT)));
    }
