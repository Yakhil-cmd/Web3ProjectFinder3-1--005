...
contract TroveManagerTester is ITroveManagerTester, TroveManager {
    ...
    function get_WHYPE() external view returns (address) {
    ...
    function getCollateralRegistry() external view returns (address) {
    ...
    function get_L_feUSDDebt() external view returns (uint256) {
        return L_feUSDDebt;
    }
    ...
    function get_lastfeUSDDebtError_Redistribution() external view returns (uint256) {
        return lastfeUSDDebtError_Redistribution;
    }
    ...
    */
    ...
    function getEffectiveRedemptionFeeInColl(uint256 _redeemAmount, uint256 _price) external view returns (uint256) {
        return collateralRegistry.getEffectiveRedemptionFeeInfeUSD(_redeemAmount) * DECIMAL_PRECISION / _price;
    }
    ...
    // Get the borrower's pending accumulated feUSD reward, earned by their stake
    function getPendingfeUSDDebtReward(uint256 _troveId) external view override returns (uint256 redistfeUSDDebtGain) {
        LatestTroveData memory trove;
        _getLatestTroveData(_troveId, trove);
        return trove.redistfeUSDDebtGain;
    }
    ...
    function getEntireDebtAndColl(uint256 _troveId)
        ...
        returns (
            uint256 entireDebt,
            uint256 entireColl,
            uint256 pendingfeUSDDebtReward,
        ...
        )
    {
        ...
        return
            (trove.entireDebt, trove.entireColl, trove.redistfeUSDDebtGain, trove.redistCollGain, trove.accruedInterest);
    }
