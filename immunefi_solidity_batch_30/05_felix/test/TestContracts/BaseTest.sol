...
import "../../src/Interfaces/IfeUSDToken.sol";
...
contract BaseTest is TestAccounts, Logging {
    ...
    IWHYPE WHYPE; // used for gas compensation
    ...
    function openTroveNoHints100pct(address _account, uint256 _coll, uint256 _feUSDAmount, uint256 _annualInterestRate)
        public
        returns (uint256 troveId)
    {
        (troveId,) = openTroveHelper(_account, 0, _coll, _feUSDAmount, _annualInterestRate);
    }
    ...
    function openTroveNoHints100pctWithIndex(
        ...
        (troveId,) = openTroveHelper(_account, _index, _coll, _feUSDAmount, _annualInterestRate);
    }
    ...
    function openTroveHelper(
        ...
        upfrontFee = predictOpenTroveUpfrontFee(_feUSDAmount, _annualInterestRate);
    ...
    function adjustTrove100pct(
        ...
        borrowerOperations.adjustTrove(
            ...
            predictAdjustTroveUpfrontFee(
                _troveId,
                _isDebtIncrease ? _feUSDChange : 0 // debtIncrease
            )
        );
    ...
    function withdrawfeUSD100pct(address _account, uint256 _troveId, uint256 _debtIncrease) public {
        vm.startPrank(_account);
        borrowerOperations.withdrawfeUSD(_troveId, _debtIncrease, predictAdjustTroveUpfrontFee(_troveId, _debtIncrease));
        vm.stopPrank();
    }

    function repayfeUSD(address _account, uint256 _troveId, uint256 _debtDecrease) public {
        vm.startPrank(_account);
        borrowerOperations.repayfeUSD(_troveId, _debtDecrease);
        vm.stopPrank();
    }
    ...
    function transferfeUSD(address _from, address _to, uint256 _amount) public {
        vm.startPrank(_from);
        feUSDToken.transfer(_to, _amount);
        vm.stopPrank();
    }
    ...
    function redeem(address _from, uint256 _feUSDAmount) public {
        vm.startPrank(_from);
        collateralRegistry.redeemCollateral(_feUSDAmount, MAX_UINT256, 1e18);
        vm.stopPrank();
    }

    function getShareofSPReward(address _depositor, uint256 _reward) public view returns (uint256) {
        return _reward * stabilityPool.getCompoundedfeUSDDeposit(_depositor) / stabilityPool.getTotalfeUSDDeposits();
    }
    ...
    function logContractAddresses() public view {
        ...
        console.log("feUSDToken addr: ", address(feUSDToken));
    }
