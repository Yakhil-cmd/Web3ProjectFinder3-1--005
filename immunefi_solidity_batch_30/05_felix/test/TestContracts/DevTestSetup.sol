...
import {BCR_ALL} from "../../src/Dependencies/Constants.sol";

contract DevTestSetup is BaseTest {

    uint256 public SP_YIELD_SPLIT = 75e16;
    ...
    function setUp() public virtual {
        ...
        (contracts, collateralRegistry, feUSDToken, hintHelpers,, WHYPE, zappers) = deployer.deployAndConnectContracts();
        ...
        whypeZapper = zappers.whypeZapper;
    ...
    function _setupForSPDepositAdjustments() internal returns (ABCDEF memory troveIDs) {
        ...
        transferfeUSD(D, A, feUSDToken.balanceOf(D) / 2);
        transferfeUSD(D, B, feUSDToken.balanceOf(D));
        ...
        returns (uint256 coll, uint256 debtRequest, ABCDEF memory troveIDs)
    {
        ...
        transferfeUSD(A, E, feUSDToken.balanceOf(A));
        transferfeUSD(B, E, feUSDToken.balanceOf(B));
        transferfeUSD(C, E, feUSDToken.balanceOf(C));
        transferfeUSD(D, E, feUSDToken.balanceOf(D));
    }
