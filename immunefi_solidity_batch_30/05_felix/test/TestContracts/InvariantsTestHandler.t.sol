...
contract InvariantsTestHandler is Assertions, BaseHandler, BaseMultiCollateralTest {
    ...
    struct ProvideToSPContext {
        TestDeployer.LiquityContractsDev c;
        uint256 pendingInterest;
        uint256 totalfeUSDDeposits;
        uint256 blockedSPYield;
        uint256 initialfeUSDDeposit;
        uint256 feUSDDeposit;
        uint256 feUSDYield;
        uint256 ethGain;
        uint256 ethStash;
        uint256 ethClaimed;
        uint256 feUSDClaimed;
        string errorString;
    }
    ...
    mapping(uint256 branchIdx => uint256) public spfeUSDDeposits;
    mapping(uint256 branchIdx => uint256) public spfeUSDYield;
    ...
    function _openTrove(OpenTroveContext memory v) internal {
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepfeUSD(msg.sender, v.borrowed);
            if (v.join) _sweepfeUSD(v.batchManager, v.batchManagementFee);
        }
    }
    ...
    function adjustTrove(
        ...
        if (!isDebtInc) debtChange = Math.min(debtChange, _handlerfeUSD);
        ...
        if (!isDebtInc) _dealfeUSD(msg.sender, debtChange);
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            if (!isDebtInc) _sweepfeUSD(msg.sender, debtChange);
        } else {
            // Cleanup (success)
            if (!isCollInc) _sweepColl(i, msg.sender, collChange);
            if (isDebtInc) _sweepfeUSD(msg.sender, debtChange);
            // Take back remaining BOLD after repayment truncation
            if (!isDebtInc) _sweepfeUSD(msg.sender, debtChange - uint256(-v.debtDelta));
            if (v.batchManager != address(0)) _sweepfeUSD(v.batchManager, v.batchManagementFee);
        }
    }
    ...
    function closeTrove(uint256 i) external {
        ...
        v.dealt = Math.min(v.t.entireDebt, _handlerfeUSD);
        _dealfeUSD(msg.sender, v.dealt);
        ...
        try v.c.borrowerOperations.closeTrove(v.troveId) {
            ...
            // Justify failures
            if (selector == BorrowerOperations.NotEnoughfeUSDBalance.selector) {
                assertLtDecimal(v.dealt, v.t.entireDebt, 18, "Shouldn't have failed as caller had enough feUSD");
            } else if (selector == BorrowerOperations.TCRBelowCCR.selector) {
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (failure)
            _sweepfeUSD(msg.sender, v.dealt);
        } else {
            // Cleanup (success)
    ...
    function batchLiquidateTroves(uint256 i) external {
        ...
        try c.troveManager.batchLiquidateTroves(_troveIdsFrom(i, l.batch)) {
            info("SP BOLD: ", c.stabilityPool.getTotalfeUSDDeposits().decimal());
            ...
            spfeUSDDeposits[i] -= l.t.spOffset;
        ...
        if (bytes(errorString).length > 0) {
            ...
            for (uint256 j = 0; j < l.batchManagers.size(); ++j) {
                _sweepfeUSD(l.batchManagers.get(j), batchManagementFee[j]);
            }
        }
    ...
    function redeemCollateral(uint256 amount, uint256 maxIterationsPerCollateral) external {
        ...
        amount = _bound(amount, 0, _handlerfeUSD);
        maxIterationsPerCollateral = _bound(maxIterationsPerCollateral, 0, maxNumTroves * 11 / 10);

        uint256 oldBaseRate = _getBaseRate();
        uint256 feUSDSupply = feUSDToken.totalSupply();
        uint256 redemptionRate = _getRedemptionRate(oldBaseRate + _getBaseRateIncrease(feUSDSupply, amount));
        ...
        try collateralRegistry.redeemCollateral(amount, maxIterationsPerCollateral, redemptionRate) {
            ...
            // Effects (global)
            _baseRate = Math.min(oldBaseRate + _getBaseRateIncrease(feUSDSupply, totalDebtRedeemed), _100pct);
        ...
        if (bytes(errorString).length > 0) {
            ...
            for (uint256 j = 0; j < branches.length; ++j) {
                ...
                for (uint256 i = 0; i < r[j].batchManagers.size(); ++i) {
                    _sweepfeUSD(r[j].batchManagers.get(i), batchManagementFee[j][i]);
                }
            }

            // There can be a slight discrepancy when hitting batched Troves
            uint256 remainingAmount = feUSDToken.balanceOf(msg.sender);
            assertApproxEqAbsDecimal(remainingAmount, amount - totalDebtRedeemed, 1e5, 18, "Wrong remaining BOLD");
            _sweepfeUSD(msg.sender, remainingAmount);
        }
    ...
    function urgentRedemption(uint256 i, uint256 amount) external {
        ...
        if (bytes(errorString).length > 0) {
            ...
            for (uint256 j = 0; j < r.batchManagers.size(); ++j) {
                _sweepfeUSD(r.batchManagers.get(j), batchManagementFee[j]);
            }
    ...
    function provideToSP(uint256 i, uint256 amount, bool claim) external {
        ...
        v.c = branches[i];
        v.pendingInterest = v.c.activePool.calcPendingAggInterest();
        v.totalfeUSDDeposits = v.c.stabilityPool.getTotalfeUSDDeposits();
        v.blockedSPYield = v.totalfeUSDDeposits < DECIMAL_PRECISION
            ? v.c.activePool.calcPendingSPYield() + v.c.stabilityPool.getYieldGainsPending()
            : 0;
        v.initialfeUSDDeposit = v.c.stabilityPool.deposits(msg.sender);
        v.feUSDDeposit = v.c.stabilityPool.getCompoundedfeUSDDeposit(msg.sender);
        v.feUSDYield = v.c.stabilityPool.getDepositorYieldGainWithPending(msg.sender);
        ...
        v.feUSDClaimed = claim ? v.feUSDYield : 0;

        info("initial deposit: ", v.initialfeUSDDeposit.decimal());
        info("compounded deposit: ", v.feUSDDeposit.decimal());
        info("yield gain: ", v.feUSDYield.decimal());
        ...
        try v.c.stabilityPool.provideToSP(amount, claim) {
            ...
            v.feUSDDeposit += amount;
            v.feUSDDeposit += v.feUSDYield;
            v.feUSDDeposit -= v.feUSDClaimed;

            // See if the change unblocked any pending yield
            v.totalfeUSDDeposits += amount;
            v.totalfeUSDDeposits += v.feUSDYield;
            v.totalfeUSDDeposits -= v.feUSDClaimed;

            uint256 newfeUSDYield =
                v.totalfeUSDDeposits >= DECIMAL_PRECISION ? v.blockedSPYield * v.feUSDDeposit / v.totalfeUSDDeposits : 0;

            assertApproxEqAbsDecimal(
                v.c.stabilityPool.getCompoundedfeUSDDeposit(msg.sender), v.feUSDDeposit, 1e7, 18, "Wrong deposit"
            );
            assertApproxEq(v.c.stabilityPool.getDepositorYieldGain(msg.sender), newfeUSDYield, 1e11, "Wrong yield gain");
            ...
            spfeUSDDeposits[i] += amount;
            spfeUSDDeposits[i] += v.feUSDYield;
            spfeUSDDeposits[i] -= v.feUSDClaimed;
            spfeUSDYield[i] -= v.feUSDYield;
        } catch Error(string memory reason) {
            v.errorString = reason;
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (failure)
            _sweepfeUSD(msg.sender, amount); // Take back the BOLD that was dealt
        } else {
            // Cleanup (success)
            _sweepfeUSD(msg.sender, v.feUSDClaimed);
    ...
    function withdrawFromSP(uint256 i, uint256 amount, bool claim) external {
        ...
        amount = _bound(amount, 0, v.feUSDDeposit * 11 / 10); // sometimes try withdrawing too much
        v.withdrawn = Math.min(amount, v.feUSDDeposit);
        ...
        try v.c.stabilityPool.withdrawFromSP(amount, claim) {
            // Preconditions
            assertGtDecimal(v.initialfeUSDDeposit, 0, 18, "Should have failed as user had zero deposit");
            assertGeDecimal(
                v.totalfeUSDDeposits + v.feUSDYield,
                v.feUSDClaimed + v.withdrawn + MIN_FEUSD_IN_SP,
                18,
                "Should have failed as withdrawal left less than MIN_FEUSD_IN_SP"
            );
            ...
            if (reason.equals("StabilityPool: User must have a non-zero deposit")) {
                assertEqDecimal(v.initialfeUSDDeposit, 0, 18, "Shouldn't have failed as user had a non-zero deposit");
            } else if (reason.equals("Withdrawal must leave totalFeUSDDeposits >= 1e18")) {
                assertLtDecimal(
                    v.totalfeUSDDeposits + v.feUSDYield,
                    v.feUSDClaimed + v.withdrawn + MIN_FEUSD_IN_SP,
                    18,
                    "Shouldn't have failed as withdrawal left more than MIN_FEUSD_IN_SP"
                );
            } else {
                revert(reason);
            }
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepfeUSD(msg.sender, v.feUSDClaimed + v.withdrawn);
            _sweepColl(i, msg.sender, v.ethClaimed);
        }
    }
    ...
    function claimAllCollGains(uint256 i) external {
        ...
        TestDeployer.LiquityContractsDev memory c = branches[i];
        uint256 pendingInterest = c.activePool.calcPendingAggInterest();
        uint256 initialfeUSDDeposit = c.stabilityPool.deposits(msg.sender);
        ...
        try c.stabilityPool.claimAllCollGains() {
            // Preconditions
            assertEqDecimal(initialfeUSDDeposit, 0, 18, "Should have failed as user had a non-zero deposit");
            assertGtDecimal(ethStash, 0, 18, "Should have failed as ETH stash was zero");

            // Effects (deposit)
            assertEqDecimal(c.stabilityPool.getCompoundedfeUSDDeposit(msg.sender), 0, 18, "Wrong deposit");
            ...
            if (reason.equals("StabilityPool: User must have no deposit")) {
                assertGtDecimal(initialfeUSDDeposit, 0, 18, "Shouldn't have failed as user had no deposit");
    ...
    function lowerBatchManagementFee(uint256 i, uint256 newManagementFee) external {
        ...
        if (bytes(errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepfeUSD(msg.sender, batchManagementFee);
        }
    }
        ...
        external
    {
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepfeUSD(v.newBatchManager, v.batchManagementFee);
        }
    }
    ...
    function removeFromBatch(uint256 i, uint256 newInterestRate, uint32 upperHintSeed, uint32 lowerHintSeed) external {
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepfeUSD(v.batchManager, v.batchManagementFee);
        }
    }
    ...
    function setBatchManagerAnnualInterestRate(
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepfeUSD(msg.sender, v.b.accruedManagementFee);
        }
    }
    ...
    function _getBaseRateIncrease(uint256 feUSDSupply, uint256 redeemed) internal pure returns (uint256) {
        return feUSDSupply > 0 ? redeemed * DECIMAL_PRECISION / feUSDSupply / REDEMPTION_BETA : 0;
    }
    ...
    function _mintYield(uint256 i, uint256 pendingInterest, uint256 upfrontFee) internal {
        ...
        uint256 mintedSPfeUSDYield = mintedYield * 75e16 / DECIMAL_PRECISION;

        spfeUSDYield[i] += mintedSPfeUSDYield;
    ...
    function _dealWETHAndApprove(address to, uint256 amount, address spender) internal {
        uint256 balance = whype.balanceOf(to);
        uint256 allowance = whype.allowance(to, spender);

        deal(address(whype), to, balance + amount);
        vm.prank(to);
        whype.approve(spender, allowance + amount);
    }
    ...
    function _dealfeUSD(address to, uint256 amount) internal {
        feUSDToken.transfer(to, amount);
        _handlerfeUSD -= amount;
    }

    function _sweepfeUSD(address from, uint256 amount) internal {
        vm.prank(from);
        feUSDToken.transfer(address(this), amount);
        _handlerfeUSD += amount;
    }
    ...
    function _aggregateLiquidation(uint256 i, LatestTroveData memory trove, LiquidationTotals storage t) internal {
        ...
        uint256 collGasComp = collRemaining / COLL_GAS_COMPENSATION_DIVISOR;
        t.collGasComp += collGasComp;
        collRemaining -= collGasComp;

        // Offset debt by SP
        uint256 spRemaining = spfeUSDDeposits[i] - t.spOffset;
        uint256 spOffset = Math.min(trove.entireDebt, spRemaining > MIN_FEUSD_IN_SP ? spRemaining - MIN_FEUSD_IN_SP : 0);
        t.spOffset += spOffset;
    ...
    function _encodeActiveTroveAdjustment(
        ...
        if (prop == AdjustedTroveProperties.onlyDebt) {
            if (isDebtIncrease) {
                return abi.encodeCall(IBorrowerOperations.withdrawfeUSD, (troveId, debtChange, maxUpfrontFee));
            } else {
                return abi.encodeCall(IBorrowerOperations.repayfeUSD, (troveId, debtChange));
            }
        }
        ...
        returns (bytes4 selector, string memory errorString)
    {
        ...
        if (revertData.length == 4) {
            ...
            if (selector == BorrowerOperations.NotEnoughfeUSDBalance.selector) {
                return (selector, "BorrowerOperations.NotEnoughfeUSDBalance()");
            }
