...
contract SPInvariantsTestHandler is BaseHandler {
    ...
    IfeUSDToken immutable feUSDToken;
    ...
    function openTrove(uint256 borrowed) external returns (uint256 debt) {
        ...
        // Sweep funds
        vm.prank(msg.sender);
        feUSDToken.transfer(address(this), borrowed);
        assertEqDecimal(feUSDToken.balanceOf(msg.sender), 0, 18, "Incomplete feUSD sweep");
        myfeUSD += borrowed;
    ...
    function provideToSp(uint256 deposited, bool useFixture) external {
        ...
        deposited = _bound(deposited, 1, myfeUSD);

        logCall("provideToSp", deposited.decimal(), "false");

        feUSDToken.transfer(msg.sender, deposited);
        vm.prank(msg.sender);
        // Provide to SP and claim Coll and BOLD gains
        stabilityPool.provideToSP(deposited, true);

        info("totalfeUSDDeposits = ", stabilityPool.getTotalfeUSDDeposits().decimal());
        _log();
        ...
        // Sweep BOLD gain
        vm.prank(msg.sender);
        feUSDToken.transfer(address(this), boldGain);
        assertEqDecimal(feUSDToken.balanceOf(msg.sender), 0, 18, "Incomplete feUSD sweep");
        myfeUSD += boldGain;
        ...
        assertEqDecimal(spfeUSD, stabilityPool.getTotalfeUSDDeposits(), 18, "Wrong SP BOLD balance");
    ...
    function liquidateMe() external {
        ...
        vm.assume(debt <= (spfeUSD > MIN_FEUSD_IN_SP ? spfeUSD - MIN_FEUSD_IN_SP : 0)); // only interested in SP offset, no redistribution
