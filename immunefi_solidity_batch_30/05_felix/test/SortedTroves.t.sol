...
import {SortedTroves, ISortedTroves, BatchId, BATCH_ID_ZERO} from "../src/SortedTroves.sol";
import {AddressesRegistry, IAddressesRegistry, IBorrowerOperations, ITroveManager} from "../src/AddressesRegistry.sol";
...
contract SortedTrovesTest is Test {
    ...
    function setUp() public {
        IAddressesRegistry addressesRegistry = IAddressesRegistry(address(new TransparentUpgradeableProxy(
            address(addressesRegistryImpl),
            address(proxyAdmin),
            ""
        )));
        addressesRegistry.initialize(150e16, 120e16, 120e16, 5e16, 10e16, 10000e18);

        ISortedTroves sortedTroves = ISortedTroves(address(new TransparentUpgradeableProxy(
            address(sortedtrovesImpl),
            address(proxyAdmin),
            ""
        )));

        tm = new MockTroveManager(SortedTroves(address(sortedTroves)));
        ...
        sortedTroves.initialize(addressesRegistry);
    }
