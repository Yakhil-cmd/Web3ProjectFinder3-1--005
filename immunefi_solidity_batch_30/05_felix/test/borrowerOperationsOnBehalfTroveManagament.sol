...
contract BorrowerOperationsOnBehalfTroveManagamentTest is DevTestSetup {
    ...
    function testCannotReopenTrove() public {
        ...
        deal(address(feUSDToken), A, troveManager.getTroveEntireDebt(ATroveId));
    ...
    function testRepayfeUSDWithAddManager() public {
        uint256 ATroveId = openTroveNoHints100pct(A, 100 ether, 10000e18, 1e17);
        ...
        // Owner can repay feUSD
        vm.startPrank(A);
        uint256 AInitialfeUSDBalance = feUSDToken.balanceOf(A);
        uint256 initialDebt = troveManager.getTroveEntireDebt(ATroveId);

        borrowerOperations.repayfeUSD(ATroveId, 10e18);
        vm.stopPrank();

        assertEq(troveManager.getTroveEntireDebt(ATroveId), initialDebt - 10e18, "Wrong trove debt 1");
        assertEq(feUSDToken.balanceOf(A), AInitialfeUSDBalance - 10e18, "Wrong owner balance 1");

        // Manager can repay feUSD
        deal(address(feUSDToken), B, 100e18);
        vm.startPrank(B);
        uint256 BInitialfeUSDBalance = feUSDToken.balanceOf(B);

        borrowerOperations.repayfeUSD(ATroveId, 10e18);
        vm.stopPrank();

        assertEq(troveManager.getTroveEntireDebt(ATroveId), initialDebt - 20e18, "Wrong trove debt 2");
        assertEq(feUSDToken.balanceOf(B), BInitialfeUSDBalance - 10e18, "Wrong manager balance 2");

        // Others can’t repay feUSD
        deal(address(feUSDToken), C, 100e18);
        vm.startPrank(C);
        uint256 CInitialfeUSDBalance = feUSDToken.balanceOf(C);
        ...
        assertEq(feUSDToken.balanceOf(C), CInitialfeUSDBalance, "Wrong manager balance 3");
    }

    function testRepayfeUSDWithoutAddManager() public {
        uint256 ATroveId = openTroveNoHints100pct(A, 100 ether, 10000e18, 1e17);
        ...
        assertEq(feUSDToken.balanceOf(A), AInitialfeUSDBalance - 10e18, "Wrong owner balance");
        vm.stopPrank();

        assertEq(borrowerOperations.addManagerOf(ATroveId), address(0));

        // Others can repay feUSD
        deal(address(feUSDToken), B, 100e18);
        uint256 BInitialfeUSDBalance = feUSDToken.balanceOf(B);
        ...
        assertEq(feUSDToken.balanceOf(B), BInitialfeUSDBalance - 10e18, "Wrong manager balance");
    }

    function testWithdrawfeUSDWithRemoveManager() public {
        uint256 ATroveId = openTroveNoHints100pct(A, 100 ether, 10000e18, 1e17);
        ...
        // Owner can withdraw feUSD
        uint256 AInitialfeUSDBalance = feUSDToken.balanceOf(A);

        withdrawfeUSD100pct(A, ATroveId, 10e18);

        assertEq(feUSDToken.balanceOf(A), AInitialfeUSDBalance + 10e18, "Wrong owner balance");

        // Manager can withdraw feUSD
        uint256 BInitialfeUSDBalance = feUSDToken.balanceOf(B);

        withdrawfeUSD100pct(B, ATroveId, 10e18);

        assertEq(feUSDToken.balanceOf(A), AInitialfeUSDBalance + 20e18, "Wrong owner balance");
        assertEq(feUSDToken.balanceOf(B), BInitialfeUSDBalance, "Wrong manager balance");
    }

    function testWithdrawfeUSDWithoutRemoveManager() public {
        uint256 ATroveId = openTroveNoHints100pct(A, 100 ether, 10000e18, 1e17);
        ...
        // Manager can’t withdraw feUSD
        uint256 BInitialfeUSDBalance = feUSDToken.balanceOf(B);

        vm.expectRevert(AddRemoveManagers.NotOwnerNorRemoveManager.selector);
        this.withdrawfeUSD100pct(B, ATroveId, 10e18);
    ...
    function testCloseTroveByManagerWithRemoveManager() public {
        ...
        deal(address(feUSDToken), B, troveManager.getTroveEntireDebt(ATroveId));
        vm.startPrank(B);
        borrowerOperations.closeTrove(ATroveId);
        vm.stopPrank();
    ...
    function testCloseTroveByOtherWithRemoveManager() public {
        ...
        deal(address(feUSDToken), C, troveManager.getTroveEntireDebt(ATroveId));
        vm.startPrank(C);
        vm.expectRevert(AddRemoveManagers.NotOwnerNorRemoveManager.selector);
        borrowerOperations.closeTrove(ATroveId);
        vm.stopPrank();
