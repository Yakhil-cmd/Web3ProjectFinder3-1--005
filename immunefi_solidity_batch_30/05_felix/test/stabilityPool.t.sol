...
contract SPTest is DevTestSetup {
    ...
    struct PTestVars {
        ...
        uint256 spEthBal1;
        uint256 spEthBal2;
        uint256 initialfeUSDGainA;
        uint256 initialfeUSDGainB;
    }
    ...
    function testProvideToSPWithClaim_WithOnlyCurrentBOLDGainsSendsTotalBOLDGainToDepositor() public {
        ...
        uint256 currentfeUSDGain = stabilityPool.getDepositorYieldGain(A);
        assertGt(currentfeUSDGain, 0);

        uint256 boldBal_A = feUSDToken.balanceOf(A);

        uint256 topUp = 1e18;
        makeSPDepositAndClaim(A, topUp);

        assertEq(feUSDToken.balanceOf(A), boldBal_A + currentfeUSDGain - topUp);
    }
    ...
    function testProvideToSPWithClaim_WithCurrentBOLDGainsZerosCurrentBOLDGains() public {
        ...
        // Check A's currentfeUSDGain reduced to 0
    ...
    function testProvideToSPNoClaimAddsBOLDGainsToDeposit() public {
        ...
        uint256 depositBefore_A = stabilityPool.getCompoundedfeUSDDeposit(A);

        uint256 topUp = 1e18;
        makeSPDepositNoClaim(A, topUp);

        assertApproximatelyEqual(
            stabilityPool.getCompoundedfeUSDDeposit(A), depositBefore_A + topUp + currentfeUSDGain, 1e4
        );
    }

    function testProvideToSPNoClaimAddsfeUSDGainsToTotalfeUSDDeposits() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        uint256 totalfeUSDDepositsBefore = stabilityPool.getTotalfeUSDDeposits();

        uint256 topUp = 1e18;
        makeSPDepositNoClaim(A, topUp);

        assertApproximatelyEqual(
            stabilityPool.getTotalfeUSDDeposits(), totalfeUSDDepositsBefore + topUp + currentfeUSDGain, 1e4
        );
    }

    function testProvideToSPNoClaimZerosCurrentfeUSDGains() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testProvideToSPNoClaimReducesDepositorfeUSDBalanceByOnlyTheTopUp() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        uint256 boldBalBefore_A = feUSDToken.balanceOf(A);

        uint256 topUp = 1e18;
        makeSPDepositNoClaim(A, topUp);

        assertEq(feUSDToken.balanceOf(A), boldBalBefore_A - topUp);
    }
    ...
    function testWithdrawFromSPWithClaim_WithCurrentBOLDGainsSendsBOLDGainToDepositor() public {
        ...
        assertEq(feUSDToken.balanceOf(A), boldBal_A + withdrawal + currentfeUSDGain);
    }

    function testWithdrawFromSPWithClaim_WithCurrentBOLDGainsZerosCurrentfeUSDGains() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testWithdrawFromSPNoClaimAddsBOLDGainsToDeposit() public {
        ...
        assertApproximatelyEqual(
            stabilityPool.getCompoundedfeUSDDeposit(A), depositBefore_A - withdrawal + currentfeUSDGain, 1e4
        );
    }

    function testWithdrawFromSPNoClaimAddsfeUSDGainsToTotalfeUSDDeposits() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        assertEq(stabilityPool.getTotalfeUSDDeposits(), totalfeUSDDepositsBefore - withdrawal + currentfeUSDGain);
    }

    function testWithdrawFromSPNoClaimZerosCurrentfeUSDGains() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testWithdrawFromSPNoClaimReducesDepositorfeUSDBalanceByOnlyTheTopUp() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        assertEq(feUSDToken.balanceOf(A), boldBalBefore_A + withdrawal);
    }
    ...
    function testClaimAllCollGainsRevertsWhenUserHasNoDeposit() public {
        ...
        // A
        uint256 compoundedDeposit_A = stabilityPool.getCompoundedfeUSDDeposit(A);
    ...
    function testClaimAllCollGainsRevertsWhenUserHasNoCollGains() public {
        ...
        // B
        uint256 compoundedDeposit_B = stabilityPool.getCompoundedfeUSDDeposit(B);
        ...
        assertEq(stabilityPool.getCompoundedfeUSDDeposit(B), 0);
    ...
    function testClaimAllCollGainsDoesNotChangeCompoundedDeposit() public {
        ...
        // A withdraws deposit
        uint256 deposit_A = stabilityPool.getCompoundedfeUSDDeposit(A);
        ...
        assertEq(compoundedDeposit_A, stabilityPool.getCompoundedfeUSDDeposit(A));
    }
    ...
    // --- feUSD reward sum 'B' tests ---

    function testfeUSDRewardsSumDoesntChangeWhenSPIsEmpty() public {
        priceFeed.setPrice(2000e18);
        openTroveNoHints100pct(A, 3 ether, 2000e18, 25e16);

        // check SP is 0
        assertEq(stabilityPool.getTotalfeUSDDeposits(), 0);
    ...
    function testfeUSDRewardSumDoesntChangeWhenNoYieldMinted() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        repayfeUSD(B, troveIDs.B, 1_000 ether);

        uint256 boldRewardSum_2 = stabilityPool.scaleToB(0);
        assertEq(boldRewardSum_2, boldRewardSum_1, "BOLD reward sum 2");
    }

    function testfeUSDRewardSumIncreasesWhenTroveOpened() public {
        _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardSumIncreasesWhenTroveInterestRateAdjusted() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardSumIncreasesWhenTroveClosed() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        vm.startPrank(F);
        feUSDToken.transfer(E, feUSDToken.balanceOf(F));
        vm.stopPrank();
    ...
    function testfeUSDRewardSumIncreasesWhenTroveDebtAndCollAdjusted() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardSumIncreasesWhenInterestAppliedPermissionlessly() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardSumIncreasesWhenTroveLiquidated() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardSumIncreasesWhenRedemptionOccurs() public {
        _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardSumIncreasesWhenNewDepositIsMade() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    function testfeUSDRewardSumIncreasesWhenDepositToppedUp() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    function testfeUSDRewardSumIncreasesWhenDepositWithdrawn() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    function testfeUSDRewardsOwedDoesntChangeWhenSPIsEmpty() public {
        priceFeed.setPrice(2000e18);
        openTroveNoHints100pct(A, 3 ether, 2000e18, 25e16);
    ...
    function testfeUSDRewardsOwedOnlyIcreasesByUpfrontFeeYieldWhenNoInterestMinted() public {
        _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardsOwedIncreasesWhenTroveOpened() public {
        _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardsOwedIncreasesWhenTroveInterestRateAdjusted() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardsOwedIncreasesWhenTroveClosed() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
        troveIDs.F = openTroveNoHints100pct(F, 3 ether, 2000e18, 25e16);
    ...
    function testfeUSDRewardsOwedIncreasesWhenTroveDebtAndCollAdjusted() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardsOwedIncreasesWhenInterestAppliedPermissionlessly() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardsOwedIncreasesWhenTroveLiquidated() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardsOwedIncreasesWhenRedemptionOccurs() public {
        _setupForSPDepositAdjustments();
    ...
    function testfeUSDRewardsOwedIncreasesWhenNewDepositIsMade() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    function testfeUSDRewardsOwedIncreasesWhenDepositToppedUp() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    function testfeUSDRewardsOwedIncreasesWhenDepositWithdrawn() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    function testGetDepositorfeUSDGain_1SPDepositor1RewardEvent_EarnsAllSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsWithoutOwedYieldRewards();

        // B withdraws entirely
        makeSPWithdrawalAndClaim(B, stabilityPool.getCompoundedfeUSDDeposit(B));

        // Check A has entirely of the non-zero SP deposits
        assertApproximatelyEqual(stabilityPool.getCompoundedfeUSDDeposit(A), stabilityPool.getTotalfeUSDDeposits(), 1e4);
        assertGt(stabilityPool.getTotalfeUSDDeposits(), 0);
    ...
    function testGetDepositorfeUSDGain_2SPDepositor1RewardEvent_EarnFairShareOfSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsWithoutOwedYieldRewards();
    ...
    function testGetDepositorfeUSDGain_1SPDepositor2RewardEvent_EarnsAllSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsWithoutOwedYieldRewards();
    ...
    function testGetDepositorfeUSDGain_2SPDepositor2RewardEvent_EarnFairShareOfSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsWithoutOwedYieldRewards();
    ...
    function testGetDepositorfeUSDGain_2SPDepositor1Liq1FreshDeposit_EarnFairShareOfSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsWithoutOwedYieldRewards();
        ...
        assertGt(expectedShareOfReward1_A, 0);
        assertGt(expectedShareOfReward1_B, 0);
        uint256 totalSPDeposits_1 = stabilityPool.getTotalfeUSDDeposits();
        ...
        uint256 totalSPDeposits_2 = stabilityPool.getTotalfeUSDDeposits();
    ...
    function testGetDepositorfeUSDGain_2SPDepositor1LiqFreshDeposit_EarnFairShareOfSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        uint256 totalSPDeposits_0 = stabilityPool.getTotalfeUSDDeposits();
        ...
        assertEq(stabilityPool.getTotalfeUSDDeposits(), 1e18, "SP total bold deposits should be 1e18");
        assertLt(stabilityPool.getCompoundedfeUSDDeposit(A), 1e18, "A should have <1e18 deposit");
        assertLt(stabilityPool.getCompoundedfeUSDDeposit(B), 1e18, "B should have <1e18 deposit");
        assertLe(stabilityPool.getCompoundedfeUSDDeposit(B) + stabilityPool.getCompoundedfeUSDDeposit(A),  1e18, "A & B deposits should sum to <=1e18");
        
        // C and D makes fresh deposit
        uint256 deposit_C = 1e18;
        uint256 deposit_D = 1e18;
        makeSPDepositAndClaim(C, deposit_C);
        transferfeUSD(C, D, deposit_D);
    ...
    function testGetDepositorfeUSDGain_2SPDepositor1LiqScaleChangeFreshDeposit_EarnFairShareOfSPYield() public {
    ...
    function testGetDepositorfeUSDGain_2SPDepositor2LiqScaleChangesFreshDeposit_EarnFairShareOfSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsBigTroves();
        ABCDEF[3] memory expectedShareOfReward;
        ...
        uint256 targetDebt_E = stabilityPool.getTotalfeUSDDeposits() - 1e9;
    ...
    function testLiquidationWithLowPDoesNotDecreaseP_Cheat_Fuzz(uint256 _cheatP, uint256 _surplus) public {
        ...
        vm.store(
            ...
            bytes32(uint256(63)), // 10th storage slot where P is stored
            bytes32(uint256(_cheatP))
        );

        // Confirm that storage slot 10 is set
        uint256 storedVal = uint256(vm.load(address(stabilityPool), bytes32(uint256(63))));
        ...
        uint256 debtDelta = troveDebt - stabilityPool.getTotalfeUSDDeposits();
    ...
    function testLiquidationsWithLowPAllowFurtherRewardsForAllFreshDepositors_Cheat_Fuzz(
        ...
        assertApproximatelyEqual(
            stabilityPool.getTotalfeUSDDeposits(), stabilityPool.getCompoundedfeUSDDeposit(A), 1e14, "A should own all SP"
        );
        ...
        transferfeUSD(B, A, feUSDToken.balanceOf(B) / 2);
        ...
        uint256 freshDeposit = feUSDToken.balanceOf(D) / 2;
        assertGt(freshDeposit, 0);

        transferfeUSD(D, C, freshDeposit);
        ...
        makeSPWithdrawalAndClaim(A, stabilityPool.getCompoundedfeUSDDeposit(A));
        assertEq(stabilityPool.getCompoundedfeUSDDeposit(A), 0, "A deposit should be zero");
        ...
        testVars.totalDepositsBefore = stabilityPool.getTotalfeUSDDeposits();
        ...
        transferfeUSD(C, E, feUSDToken.balanceOf(C));
        makeSPDepositAndClaim(E, feUSDToken.balanceOf(E));
        ...
        public
    {
        ...
        ABCDEF memory troveIDs = _setupForPTests();
        testVars.initialfeUSDGainA = stabilityPool.getDepositorYieldGain(A);
        testVars.initialfeUSDGainB = stabilityPool.getDepositorYieldGain(B);
        ...
        testVars.totalSPBeforeLiq_C = stabilityPool.getTotalfeUSDDeposits();
        testVars.deposit1_A = stabilityPool.getCompoundedfeUSDDeposit(A);
        testVars.deposit1_B = stabilityPool.getCompoundedfeUSDDeposit(B);

        uint256 spEthBalBefore = collToken.balanceOf(address(stabilityPool));
        liquidate(A, troveIDs.C);
        bool spTooLowAfterLiquidateA = stabilityPool.getTotalfeUSDDeposits() < DECIMAL_PRECISION;
        ...
        assertGt(stabilityPool.getCompoundedfeUSDDeposit(A), 0);
        assertGt(stabilityPool.getCompoundedfeUSDDeposit(B), 0);

        testVars.deposit2_A = stabilityPool.getCompoundedfeUSDDeposit(A);
        testVars.deposit2_B = stabilityPool.getCompoundedfeUSDDeposit(B);
        ...
        transferfeUSD(B, D, feUSDToken.balanceOf(B));
        makeSPDepositAndClaim(D, feUSDToken.balanceOf(D));
        testVars.totalSPBeforeLiq_D = stabilityPool.getTotalfeUSDDeposits();
        ...
        assertApproximatelyEqual(testVars.expectedDeposit2_A, stabilityPool.getCompoundedfeUSDDeposit(A), 1e18);
        assertApproximatelyEqual(testVars.expectedDeposit2_B, stabilityPool.getCompoundedfeUSDDeposit(B), 1e18);
        assertGt(stabilityPool.getCompoundedfeUSDDeposit(A), 0);
        assertGt(stabilityPool.getCompoundedfeUSDDeposit(B), 0);
        ...
        assertApproximatelyEqual(
            testVars.initialfeUSDGainA + testVars.expectedShareOfYield1_A,
            testVars.boldGainA,
            1e4,
            "A yield gain mismatch"
        );
        assertApproximatelyEqual(
            testVars.initialfeUSDGainB + testVars.expectedShareOfYield1_B,
            testVars.boldGainB,
            1e4,
    ...
    // --- Liq debt x in range: (totalfeUSDDeposits - 1e18) < x <= totalfeUSDDeposits ---
    ...
    function testLiqDebtWithin1BOLDOfTotalDepositsLeaves1BOLDinSP_Fuzz(uint256 _toRedist) public {
        ...
        // B's target debt should be: totalfeUSDDeposits - 1e18 + _toRedist
        ...
        assertLt(stabilityPool.getTotalfeUSDDeposits() - troveManager.getTroveEntireDebt(troveIdB), 1e18);
        ...
        assertEq(stabilityPool.getTotalfeUSDDeposits(), 1e18);
        // A's deposit after liq should be <= totalfeUSDDeposits, and very close to it, due to rounding error
        assertLe(stabilityPool.getCompoundedfeUSDDeposit(A), stabilityPool.getTotalfeUSDDeposits());
        assertApproximatelyEqual(stabilityPool.getCompoundedfeUSDDeposit(A), stabilityPool.getTotalfeUSDDeposits(), 1e3); 
    }
    ...
    function testLiqDebtWithin1feUSDOfTotalDepositsSplitsCollGainsCorrectly_Fuzz() public {
        ...
        // Confirm that B's entire debt is within 1e18 of SP total deposits
        assertLt(stabilityPool.getTotalfeUSDDeposits() - vars.debtBBefore, 1e18);
        ...
        assertEq(collToken.balanceOf(C), vars.collCBefore + vars.collGasComp + 0, "Incorrect coll increase C");
        ...
        assertApproxEqAbs(
            vars.spCollBalAfter - vars.spCollBalBefore,
            vars.collToGiveToSP,
            10,
            "Incorrect coll in SP"
        );
    ...
    // --- Liq debt x in range: x < (totalfeUSDDeposits - 1e18)
    ...
    function testLiqDebtLessThanSPMinus1BOLDLeavesSurplusInSP_Fuzz(uint256 _surplusToLeaveInSP) public {
        ...
        // Confirm that B's entire debt is less than (totalfeUSDDeposits - 1e18)
        assertLe(troveManager.getTroveEntireDebt(troveIdB), stabilityPool.getTotalfeUSDDeposits() - 1e18);
        ...
        assertEq(stabilityPool.getTotalfeUSDDeposits(), _surplusToLeaveInSP);
        // A's deposit after liq should be <= totalfeUSDDeposits, and very close to it, due to rounding error
        assertLe(stabilityPool.getCompoundedfeUSDDeposit(A), stabilityPool.getTotalfeUSDDeposits());
        assertApproximatelyEqual(stabilityPool.getCompoundedfeUSDDeposit(A), stabilityPool.getTotalfeUSDDeposits(), 1e3); 
    }
    ...
    // --- Liq debt x for x > totalfeUSDDeposits ---
    ...
    function testLiqDebtGreaterThanTotalSPLeaves1BOLDInSP_Fuzz(uint256 _toRedist) public {
        ...
        // Confirm that B's entire debt is greater than totalfeUSDDeposits 
        assertGt(troveManager.getTroveEntireDebt(troveIdB), stabilityPool.getTotalfeUSDDeposits());
    ...
    function testLiqDebtGreaterThanTotalSSplitsCollGainsCorrectly_Fuzz(uint256 _toRedist) public {
        ...
        uint256 spBefore = stabilityPool.getTotalfeUSDDeposits();
        ...
        assertEq(stabilityPool.getTotalfeUSDDeposits(), spBefore - liqDebtOffset);
       ...
       // Check coll redistribution is correct
        ...
        // Check C's total WETH balance increased by the total gas comp
        assertEq(collToken.balanceOf(C), vars.collCBefore + vars.collGasComp + 0);
    ...
    function testSingleDepositorCantWithdrawWhenLeavingLessThan1feUSDInPool_Fuzz(uint256 _withdrawal) public {
        priceFeed.setPrice(3000e18);
        ...
        assertEq(stabilityPool.getCompoundedfeUSDDeposit(A), 2e18);
        assertEq(stabilityPool.getTotalfeUSDDeposits(), 2e18);

        vm.startPrank(A);
        vm.expectRevert("Withdrawal must leave totalFeUSDDeposits >= 1e18");
        // Claim all gains here so they don't get added to deposit
        stabilityPool.withdrawFromSP(_withdrawal, true);
    }

    function testSingleDepositorCanWithdrawWhenLeavingAtLeast1feUSDInPool_Fuzz(uint256 _withdrawal) public {
        priceFeed.setPrice(3000e18);
        openTroveNoHints100pct(A, 5 ether, 2000e18, 5e16);
        ...
        uint256 ABalBefore = feUSDToken.balanceOf(A);
        ...
        assertEq(stabilityPool.getCompoundedfeUSDDeposit(A), 2e18 - _withdrawal);
        assertEq(feUSDToken.balanceOf(A), ABalBefore + _withdrawal + accruedYieldGains);

        // Confirm SP has >= 1e18
        assertGe(stabilityPool.getTotalfeUSDDeposits(), 1e18); 
    }

    // Tests TODO:
    // - Collateral gains split correctly across mixed offset + redist
    // - Liquidations when SP.totalfeUSDDeposits <1e18 
    // - Depleted deposits can't withdraw if would leave <1e18 in SP
    // - When A can't withdraw, B can top up SP, then A can withdraw
    // - feUSD yield gains added to depsosit, allowing withdrawal
}
