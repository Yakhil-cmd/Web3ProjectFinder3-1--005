...
contract Redemptions is DevTestSetup {
    ...
    function testRedemptionIncludesAccruedTroveInterest() public {
        ...
        assertEq(trove_A.redistfeUSDDebtGain, 0);
    ...
    function testRedemption1TroveLeavesCollFeeInTrove() public {
        ...
        // Expect Trove's coll reduced by the Coll corresponding to the feUSD redeemed (less the Coll fee)
    ...
    function testRedemption3TroveLeavesCollFeesInTroves() public {
        ...
        feUSDRedeemAmounts memory feUSDRedeemAmounts;
        CorrespondingColl memory correspondingColl;

        feUSDRedeemAmounts.A = troveManager.getTroveDebt(troveIDs.A);
        feUSDRedeemAmounts.B = troveManager.getTroveDebt(troveIDs.B);
        feUSDRedeemAmounts.C = troveManager.getTroveDebt(troveIDs.C) / 2;
        uint256 totalfeUSDRedeemAmount = feUSDRedeemAmounts.A + feUSDRedeemAmounts.B + feUSDRedeemAmounts.C;

        correspondingColl.A = feUSDRedeemAmounts.A * DECIMAL_PRECISION / price;
        correspondingColl.B = feUSDRedeemAmounts.B * DECIMAL_PRECISION / price;
        correspondingColl.C = feUSDRedeemAmounts.C * DECIMAL_PRECISION / price;

        uint256 redemptionFeePct = collateralRegistry.getEffectiveRedemptionFeeInfeUSD(totalfeUSDRedeemAmount)
            * DECIMAL_PRECISION / totalfeUSDRedeemAmount;
        ...
        // Expect each Trove's coll to reduce by the Coll corresponding to the feUSD redeemed, less the Coll fee
        ...
        redeem(E, totalfeUSDRedeemAmount);
    ...
    function testRedemption3TroveLeavesCollFeesInActivePool() public {
        ...
        uint256 totalfeUSDRedeemAmount = feUSDRedeemAmounts.A + feUSDRedeemAmounts.B + feUSDRedeemAmounts.C;
        uint256 totalCorrespondingColl = totalfeUSDRedeemAmount * DECIMAL_PRECISION / price;
    ...
    function testZombieTrovePointerGetsResetIfTroveIsClosed() public {
        ...
        deal(address(feUSDToken), B, debt_B);
    ...
    function testZombieTrovePointerGetsResetIfTroveIsLiquidated() public {
        ...
        priceFeed.setPrice(priceFeed.getPrice() / 60);
    ...
    function testZombieTrovePointerIsPreservedIfItIsSkippedAndNoNewZombieIsProduced() external {
        ...
        (uint256 trove1,) = openTroveWithExactICRAndDebt(A, 0, 1.1 ether, 1_000 ether, 0.01 ether); // ICR 110%
        openTroveWithExactICRAndDebt(A, 1, 1.5 ether, 2_000 ether, 0.02 ether); // ICR 150%
    ...
    function testZombieBorrowerCanCloseZombieTrove() public {
        ...
        // E sends feUSD back to A and B so they can close
        transferfeUSD(E, A, feUSDToken.balanceOf(E) / 2);
        transferfeUSD(E, B, feUSDToken.balanceOf(E));
    ...
    function testZombieBorrowerCanDrawFreshDebtToAboveMIN_DEBT() public {
        ...
        // A and B withdraw feUSD from their zombie Trove
    ...
    function testZombieTroveBorrowerCanNotDrawFreshDebtToBelowMIN_DEBT() public {
        ...
        // A and B attempt to withdraw feUSD, but not enough
    ...
    function testZombieTroveBorrowerCanNotRepayDebt() public {
        ...
        // E sends feUSD back to A and B
        transferfeUSD(E, A, feUSDToken.balanceOf(E) / 2);
        transferfeUSD(E, B, feUSDToken.balanceOf(E));

        vm.startPrank(A);
        vm.expectRevert(BorrowerOperations.TroveNotActive.selector);
        borrowerOperations.repayfeUSD(troveIDs.A, debtRepayment);
        vm.stopPrank();

        vm.startPrank(B);
        vm.expectRevert(BorrowerOperations.TroveNotActive.selector);
        borrowerOperations.repayfeUSD(troveIDs.B, debtRepayment);
        vm.stopPrank();
    }

    function testZombieTroveBorrowerCanNotUseNormalWithdrawfeUSDFunction() public {
        (,, ABCDEF memory troveIDs) = _setupForRedemptionAscendingInterest();
        ...
        vm.startPrank(A);
        vm.expectRevert(BorrowerOperations.TroveNotActive.selector);
        borrowerOperations.withdrawfeUSD(troveIDs.A, debtWithdrawal, 0);
        vm.stopPrank();

        vm.startPrank(B);
        vm.expectRevert(BorrowerOperations.TroveNotActive.selector);
        borrowerOperations.withdrawfeUSD(troveIDs.B, debtWithdrawal, 0);
        vm.stopPrank();
    }
    ...
    function testZombieTroveCanBeLiquidated() public {
        ...
        assertGt(stabilityPool.getTotalfeUSDDeposits(), troveManager.getTroveEntireDebt(troveIDs.B));
