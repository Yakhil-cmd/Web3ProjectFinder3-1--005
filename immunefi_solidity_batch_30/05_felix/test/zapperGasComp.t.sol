...
contract ZapperGasCompTest is DevTestSetup {
    function setUp() public override {
        ...
        troveManagerParams[1] = TestDeployer.TroveManagerParams(160e16, 120e16, 120e16, 5e16, 10e16, 10_000_000 ether);
    ...
    function testCanOpenTrove() external {
        ...
        vm.startPrank(A);
        uint256 troveId = gasCompZapper.openTroveWithRawHYPE{value: ETH_GAS_COMPENSATION}(params);
        vm.stopPrank();
    ...
    function testCanRepayfeUSD() external {
        ...
        // Approve and repay
        vm.startPrank(B);
        feUSDToken.approve(address(gasCompZapper), feUSDAmount2);
        gasCompZapper.repayfeUSD(troveId, feUSDAmount2);
        vm.stopPrank();
    ...
    function testCanWithdrawfeUSD() external {
        ...
        // Withdraw feUSD
        vm.startPrank(B);
        gasCompZapper.withdrawfeUSD(troveId, feUSDAmount2, feUSDAmount2);
        vm.stopPrank();
    ...
    function testCanAdjustTroveWithdrawCollAndfeUSD() external {
        ...
        vm.startPrank(B);
        gasCompZapper.adjustTrove(troveId, collAmount2, false, feUSDAmount2, true, feUSDAmount2);
        vm.stopPrank();
    ...
    function testCanAdjustZombieTroveWithdrawCollAndfeUSD() external {
        ...
        // Adjust (withdraw coll and feUSD)
        vm.startPrank(B);
        gasCompZapper.adjustZombieTrove(troveId, collAmount2, false, feUSDAmount2, true, 0, 0, feUSDAmount2);
        vm.stopPrank();
    ...
    function testCanCloseTrove() external {
        ...
        vm.startPrank(B);
        deal(address(WHYPE), B, ETH_GAS_COMPENSATION);
        WHYPE.approve(address(borrowerOperations), ETH_GAS_COMPENSATION);
        ...
        vm.startPrank(A);
        feUSDToken.approve(address(gasCompZapper), type(uint256).max);
        gasCompZapper.closeTroveToRawHYPE(troveId);
        vm.stopPrank();
    ...
    function testExcessRepaymentByAdjustGoesBackToUser() external {
        ...
        assertEq(feUSDToken.balanceOf(address(gasCompZapper)), 0, "Zapper feUSD bal should be zero");
        assertEq(A.balance, ethBalanceBefore, "ETH bal mismatch");
    ...
    function testExcessRepaymentByRepayGoesBackToUser() external {
        ...
        vm.startPrank(A);
        feUSDToken.approve(address(gasCompZapper), type(uint256).max);
        gasCompZapper.repayfeUSD(troveId, 9900e18);
        vm.stopPrank();
