...
contract ShutdownTest is DevTestSetup {
    ...
    function setUp() public override {
        ...
        SP_YIELD_SPLIT = vm.envUint("SP_YIELD_SPLIT");
    ...
    function testCannotWithdrawfeUSDAfterShutdown() public {
        uint256 troveId = prepareAndShutdownFirstBranch();

        vm.startPrank(A);
        vm.expectRevert(BorrowerOperations.IsShutDown.selector);
        borrowerOperations.withdrawfeUSD(troveId, 1, 10000e18);
        vm.stopPrank();
    }

    function testCannotRepayfeUSDAfterShutdown() public {
        uint256 troveId = prepareAndShutdownFirstBranch();

        vm.startPrank(A);
        vm.expectRevert(BorrowerOperations.IsShutDown.selector);
        borrowerOperations.repayfeUSD(troveId, 1000e18);
        vm.stopPrank();
    }
    ...
    function testCannotAdjustZombieTroveAfterShutdown() public {
        ...
        //deal(address(feUSDToken), B, 20000e18);
        vm.startPrank(B);
    ...
    function testCanCloseTroveAfterShutdown() public {
        ...
        deal(address(feUSDToken), A, 20000e18);
        vm.startPrank(A);
    ...
    function testCannotUrgentRedeemWithoutShutdown() public {
        ...
        vm.startPrank(A);
        vm.expectRevert(); // changed because of assembly revert
    ...
    function testCanUrgentReedemPartiallyBelow100() external {
        ...
        uint256 feUSDBalanceBefore = feUSDToken.balanceOf(A);
        ...
        assertEq(feUSDToken.balanceOf(A), feUSDBalanceBefore - redemptionAmount, "feUSD balance mismatch");
        assertEq(
            contractsArray[0].collToken.balanceOf(A),
            collBalanceBefore + redemptionAmount * DECIMAL_PRECISION / price * (DECIMAL_PRECISION + URGENT_REDEMPTION_BONUS)
                    / DECIMAL_PRECISION,
    ...
    function testCanUrgentReedemFullyBelow100() external {
        ...
        assertEq(
            feUSDToken.balanceOf(A),
            feUSDBalanceBefore - 11e18 * price / DECIMAL_PRECISION * DECIMAL_PRECISION / (DECIMAL_PRECISION + URGENT_REDEMPTION_BONUS),
            "feUSD balance mismatch"
        );
    ...
    function testShutdownZerosMintsPendingAggInterestToSP() public {
        ...
        uint256 spBal1 = feUSDToken.balanceOf(address(contractsArray[0].stabilityPool));
        ...
        uint256 spBal2 = feUSDToken.balanceOf(address(contractsArray[0].stabilityPool));
    ...
    function testUrgentRedemptionMintsNoInterestToSP() public {
        ...
        uint256 spfeUSDBal_t0 = feUSDToken.balanceOf(address(contractsArray[0].stabilityPool));
        ...
        uint256 spfeUSDBal_t1 = feUSDToken.balanceOf(address(contractsArray[0].stabilityPool));

        // Check SP feUSD bal didnt change from urgent redemption
        assertEq(spfeUSDBal_t1, spfeUSDBal_t0);
    }
