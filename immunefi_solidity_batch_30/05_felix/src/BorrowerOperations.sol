...
import "./Libraries/BorrowerOperationsInit.sol";


contract BorrowerOperations is Initializable, LiquityBase, AddRemoveManagers, IBorrowerOperations {
    ...
    */
    ...
    // Admin shutdown which is reversible
    bool public isTemporaryShutdown;
    ...
    error MaxDebtCapReached();
    error TemporaryShutdown();
    error IsNotTemporaryShutdown();
    ...
    event AdminShutdown(uint256 _timestamp);
    event ShutdownResumed(uint256 _timestamp);
    ...
    function initialize(IAddressesRegistry _addressesRegistry) external initializer {
        BorrowerOperationsInit.initialize(_addressesRegistry);
    }
    ...
    function setMaxDebtCap(uint256 _maxDebtCap) external {
        _requireCallerIsAdmin();
        assembly {
            sstore(maxDebtCap.slot, _maxDebtCap)
        }
    }
    ...
    function openTrove(
        ...
        _requireMaxCapNotExceeded(_feUSDAmount);
        ...
        returns (uint256)
    {
        ...
        _requireMaxCapNotExceeded(_params.feUSDAmount);
    ...
    function _openTrove(
        ...
        _requireIsNotTemporaryShutdown();
        ...
        _change.debtIncrease = _feUSDAmount;
        ...
        // Mint the requested _feUSDAmount to the borrower and mint the gas comp to the GasPool
        vars.feUSDToken.mint(msg.sender, _feUSDAmount);
        WHYPE.transferFrom(msg.sender, gasPoolAddress, ETH_GAS_COMPENSATION);
    ...
    // Withdraw feUSD tokens from a trove: mint new feUSD tokens to the owner, and increase the trove's debt accordingly
    function withdrawfeUSD(uint256 _troveId, uint256 _feUSDAmount, uint256 _maxUpfrontFee) external override {
        ITroveManager troveManagerCached = troveManager;
        _requireTroveIsActive(troveManagerCached, _troveId);
        _requireMaxCapNotExceeded(_feUSDAmount);

        TroveChange memory troveChange;
        troveChange.debtIncrease = _feUSDAmount;
        _adjustTrove(troveManagerCached, _troveId, troveChange, _maxUpfrontFee);
    }

    // Repay feUSD tokens to a Trove: Burn the repaid feUSD tokens, and reduce the trove's debt accordingly
    function repayfeUSD(uint256 _troveId, uint256 _feUSDAmount) external override {
        ITroveManager troveManagerCached = troveManager;
        _requireTroveIsActive(troveManagerCached, _troveId);

        TroveChange memory troveChange;
        troveChange.debtDecrease = _feUSDAmount;
    ...
    function _initTroveChange(
        ...
        if (_isDebtIncrease) {
            _troveChange.debtIncrease = _feUSDChange;
        } else {
            _troveChange.debtDecrease = _feUSDChange;
        }
    }
    ...
    function adjustTrove(
        ...
        _requireTroveIsActive(troveManagerCached, _troveId);
        if(_isDebtIncrease){
            _requireMaxCapNotExceeded(_feUSDChange);
        }

        TroveChange memory troveChange;
        _initTroveChange(troveChange, _collChange, _isCollIncrease, _feUSDChange, _isDebtIncrease);
        _adjustTrove(troveManagerCached, _troveId, troveChange, _maxUpfrontFee);
    }
    ...
    */
    function _adjustTrove(
        ...
        // When the adjustment is a debt repayment, check it's a valid amount and that the caller has enough feUSD
        if (_troveChange.debtDecrease > 0) {
            ...
            _requireSufficientfeUSDBalance(vars.feUSDToken, msg.sender, _troveChange.debtDecrease);
        }
        ...
        if (isTroveInBatch) {
            ...
            batchFutureDebt = batch.entireDebtWithoutRedistribution + vars.trove.redistfeUSDDebtGain
                + _troveChange.debtIncrease - _troveChange.debtDecrease;

            _troveChange.appliedRedistfeUSDDebtGain = vars.trove.redistfeUSDDebtGain;
        ...
        _moveTokensFromAdjustment(receiver, _troveChange, vars.feUSDToken, vars.activePool);
    }

    function closeTrove(uint256 _troveId) external override {
        ITroveManager troveManagerCached = troveManager;
        IActivePool activePoolCached = activePool;
        IfeUSDToken feUSDTokenCached = feUSDToken;
        ...
        _requireSufficientfeUSDBalance(feUSDTokenCached, msg.sender, trove.entireDebt);
        ...
        if (batchManager != address(0)) {
            ...
            uint256 batchFutureDebt =
                batch.entireDebtWithoutRedistribution - (trove.entireDebt - trove.redistfeUSDDebtGain);
        ...
        WHYPE.transferFrom(gasPoolAddress, receiver, ETH_GAS_COMPENSATION);
        // Burn the remainder of the Trove's entire debt from the user
        feUSDTokenCached.burn(msg.sender, trove.entireDebt);
    ...
    function applyPendingDebt(uint256 _troveId, uint256 _lowerHint, uint256 _upperHint) public {
        ...
        change.appliedRedistfeUSDDebtGain = trove.redistfeUSDDebtGain;
        ...
        if (batchManager == address(0)) {
            ...
            change.newWeightedRecordedDebt =
                (batch.entireDebtWithoutRedistribution + trove.redistfeUSDDebtGain) * batch.annualInterestRate;
            ...
            change.newWeightedRecordedBatchManagementFee =
                (batch.entireDebtWithoutRedistribution + trove.redistfeUSDDebtGain) * batch.annualManagementFee;
        }
    ...
    function setInterestBatchManager(
        ...
        newBatchTroveChange.appliedRedistfeUSDDebtGain = vars.trove.redistfeUSDDebtGain;
    ...
    function removeFromBatch(
        ...
        uint256 batchFutureDebt =
            vars.batch.entireDebtWithoutRedistribution - (vars.trove.entireDebt - vars.trove.redistfeUSDDebtGain);

        TroveChange memory batchChange;
        batchChange.appliedRedistfeUSDDebtGain = vars.trove.redistfeUSDDebtGain;
    ...
    function _applyUpfrontFee(
        ...
        // ICR is based on the requested feUSD amount + upfront fee.
    ...
    function resumeFromShutdown() external {
        _requireCallerIsAdmin();
        if (!isTemporaryShutdown) revert IsNotTemporaryShutdown();
        assembly {
            sstore(isTemporaryShutdown.slot, false)
        }
        emit ShutdownResumed(block.timestamp);
    }
    ...
    function adminShutdown() external {
        _requireCallerIsAdmin();
        if (isTemporaryShutdown || hasBeenShutDown) revert IsShutDown();

        assembly {
            sstore(isTemporaryShutdown.slot, true)
        }

        emit AdminShutdown(block.timestamp);
    }

    // Not technically a "Borrower op", but seems best placed here given current shutdown logic.
    function shutdownFromOracleFailure(address _failedOracleAddr) external virtual {
        ...
        if (hasBeenShutDown || isTemporaryShutdown) return;
    ...
    function _applyShutdown() internal {
        assembly {
            mstore(0x00, 0x1bfa0d7b) // mintAggInterest()
            // We need to shift the address by 16 bits to the right to get the correct address (0slot ---> address, bool, uint8)
            let success := call(gas(), shr(16, sload(activePool.slot)), 0x00, 28, 32, 0x00, 0x20) // gas, address, value, offset, size, returnDataOffset, returnDataSize (fix the correct address retrieval)
            if iszero(success) {
                revert(0, 0)
            }
            sstore(hasBeenShutDown.slot, 1)
            mstore(0x00, 0xfc0e74d1) // shutdown()
            success := call(gas(), sload(troveManager.slot), 0x00, 28, 32, 0x00, 0x20)// gas, address, value, offset, size, returnDataOffset, returnDataSize
            if iszero(success) {
                revert(0, 0)
            }
        }
    ...
    // This function mints the feUSD corresponding to the borrower's chosen debt increase
    ...
    function _moveTokensFromAdjustment(
        ...
        if (_troveChange.debtIncrease > 0) {
            _feUSDToken.mint(withdrawalReceiver, _troveChange.debtIncrease);
        } else if (_troveChange.debtDecrease > 0) {
            _feUSDToken.burn(msg.sender, _troveChange.debtDecrease);
        }
    ...
    function _requireIsNotTemporaryShutdown() internal view {
        // if(isTemporaryShutdown) {
        //     revert TemporaryShutdown();
        // }
        assembly {
            if sload(isTemporaryShutdown.slot) {
                mstore(0x00, 0xec2a2fd2)
                revert(0x1c, 0x04)
            }
        }
    }

    function _requireMaxCapNotExceeded(uint256 _newDebt) internal view {
        if(getEntireBranchDebt() + _newDebt > maxDebtCap){
            revert MaxDebtCapReached();
        }
    }

    function _requireCallerIsAdmin() internal view {
        assembly {
            // Load the TroveManager address from storage
            let _troveManager := sload(troveManager.slot)
            
            // Prepare the function selector for `collateralRegistry()`
            mstore(0x00, 0xd330fadd) 
            
            // Call the collateralRegistry() function on troveManager
            let success := staticcall(gas(), _troveManager, 28, 32, 0x00, 0x20)
            if iszero(success) {
                revert(0, 0)
            }
            
            // Load the collateralRegistry address from the return data
            let collRegistry := mload(0x00)
            
            // Prepare the function selector for `owner()`
            mstore(0x00, 0x8da5cb5b)
            
            // Call the owner() function on collRegistry
            success := staticcall(gas(), collRegistry, 28, 32, 0x00, 0x20)
            if iszero(success) {
                revert(0, 0)
            }
            
            // Load the owner address from the return data
    ...
    function _requireIsNotInBatch(uint256 _troveId) internal view {
        ...
        assembly {
            ...
            mstore(0x00, _troveId)
            mstore(0x20, interestBatchManagerOf.slot)
            let slot := keccak256(0x00, 0x40)
            
            // Load value and check if not zero
            if iszero(eq(sload(slot), 0)) {
                mstore(0x00, 0x4742bbb3)  // TroveInBatch()
                revert(0x1c, 0x04)
            }
        }
    }

    function _requireIsInBatch(uint256 _troveId) internal view returns (address batchManager) {
        // address batchManager = interestBatchManagerOf[_troveId];
        // if (batchManager == address(0)) {
        ...
        assembly {
            ...
            let slot := keccak256(0x00, 0x40)
            if eq(sload(slot), 0) {
                mstore(0x00, 0x93f3f3c1)  // TroveInBatch()
                revert(0x1c, 0x04)
            }
            batchManager := sload(slot)
        }
    }
    ...
    function _requireNonZeroDebt(uint256 _troveDebt) internal pure {
        ...
        // }
        assembly {
            if iszero(_troveDebt){
                mstore(0x00, 0x16f6fdf8)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireValidAdjustmentInCurrentMode(
        ...
        /*
        * Below Critical Threshold, it is not permitted:uint256 constant MIN_BOLD_IN_SP = 1e18;
        *
    ...
    function _requireICRisAboveMCR(uint256 _newICR) internal view {
        ...
        // }
        assembly {
            if lt(_newICR, sload(MCR.slot)){
                mstore(0x00, 0x4d4556d8)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireICRisAboveMCRPlusBCR(uint256 _newICR) internal view {
        ...
        uint256 _BCR = BCR;
        assembly {
            if lt(_newICR, add(sload(MCR.slot), _BCR)) {
                mstore(0x00, 0x4b4bf81c)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireNoBorrowingUnlessNewTCRisAboveCCR(uint256 _debtIncrease, uint256 _newTCR) internal view {
        ...
        // }
        assembly {
            if and(gt(_debtIncrease, 0), lt(_newTCR, sload(CCR.slot))){
                mstore(0x00, 0xc855c3b3)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireDebtRepaymentGeCollWithdrawal(TroveChange memory _troveChange, uint256 _price) internal pure {
        ...
        // }
        uint256 _debtDecrease = _troveChange.debtDecrease;
        uint256 _collDecrease = _troveChange.collDecrease;
        uint256 _decimals = DECIMAL_PRECISION;
        assembly {
            if lt(mul(_debtDecrease, _decimals), mul(_collDecrease, _price)) {
                mstore(0x00, 0xd6769563)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireAtLeastMinDebt(uint256 _debt) internal pure {
        ...
        // }
        assembly {
            if lt(_debt, MIN_DEBT) {
                mstore(0x00, 0xf1e41913)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireValidCollWithdrawal(uint256 _currentColl, uint256 _collWithdrawal) internal pure {
        ...
        // }
        assembly {
            if gt(_collWithdrawal, _currentColl) {
                mstore(0x00, 0xb30a1bc9)
                revert(0x1c, 0x04)
            }
        }
    }

    function _requireSufficientfeUSDBalance(IfeUSDToken _feUSDToken, address _borrower, uint256 _debtRepayment)
        internal
        view
    {
        // if (_feUSDToken.balanceOf(_borrower) < _debtRepayment) { // 0x70a08231
        //     revert NotEnoughfeUSDBalance();
        // }
        ...
        assembly {
            ...
            let _balance := mload(0x00)
            if lt(_balance, _debtRepayment) {
                mstore(0x00, 0x241a20ee)
                revert(0x1c, 0x04)
            }
        }
    }
    function _requireValidAnnualInterestRate(uint256 _annualInterestRate) internal pure {
        ...
        // }
        uint256 _minAnnualInterestRate = MIN_ANNUAL_INTEREST_RATE;
        uint256 _maxAnnualInterestRate = MAX_ANNUAL_INTEREST_RATE;
        assembly {
            if lt(_annualInterestRate, _minAnnualInterestRate) {
                mstore(0x00, 0xd2693ab0)
                revert(0x1c, 0x04)
            }
            if gt(_annualInterestRate, _maxAnnualInterestRate) {
                mstore(0x00, 0x40c2ffa4)
                revert(0x1c, 0x04)
            }
        }
    }
        ...
        pure
    {
        ...
        // }
        assembly {
            if eq(_oldAnnualInterestRate, _newAnnualInterestRate) {
                mstore(0x00, 0x8a00f924)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireOrderedRange(uint256 _minInterestRate, uint256 _maxInterestRate) internal pure {
        ...
        assembly {
            if or(gt(_minInterestRate, _maxInterestRate), eq(_minInterestRate, _maxInterestRate)){
                mstore(0x00, 0x2a2b2ad1)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireInterestRateInRange(
        ...
        // }
        assembly {
            if or(gt(_minInterestRate, _annualInterestRate), gt(_annualInterestRate, _maxInterestRate)) {
                mstore(0x00, 0x9736ee75)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireDelegateInterestRateChangePeriodPassed(
        ...
        // }
        assembly {
            if lt(timestamp(), add(_lastInterestRateAdjTime, _minInterestRateChangePeriod)) {
                mstore(0x00, 0x85100883)
                revert(0x1c, 0x04)
            }
        }
    }
        ...
        pure
    {
        ...
        // }
        assembly {
            if eq(_oldBatchManagerAddress, _newBatchManagerAddress) {
                mstore(0x00,0x1c56f503)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireCallerIsTroveManager() internal view {
        assembly {
            if iszero(eq(caller(), sload(troveManager.slot))) {
                mstore(0x00, 0xe2ab44a8) // CallerNotTroveManager()
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireCallerIsPriceFeed() internal view {
        assembly {
            if iszero(eq(caller(), sload(priceFeed.slot))) {
                mstore(0x00, 0x8d3c07a8) // CallerNotPriceFeed()
                revert(0x1c, 0x04)
            }
        }
    }

    function _requireOraclesLive() internal returns (uint256 price) {
        assembly {
            mstore(0x00, 0x0fdb11cf) // fetchPrice()
            let success := call(gas(), sload(priceFeed.slot), 0x00, 28, 32, 0x00, 0x40) // gas, address, offset, size, returnDataOffset, returnDataSize
            if iszero(success) {
                revert(0, 0)
            }
            if mload(0x20) {
                mstore(0x00, 0x23656466) // NewOracleFailureDetected()
                revert(0x1c, 0x04)
            }
            price := mload(0x00)
        }
