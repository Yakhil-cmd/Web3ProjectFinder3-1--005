...
contract GasCompZapper is BaseZapper {
    ...
    function __GasCompZapper_init(
        ...
        require(address(WHYPE) != address(collToken), "GCZ: Wrong coll branch");

        // Approve WHYPE to BorrowerOperations
    ...
    function _adjustTrovePre(
        ...
        address receiver =
            _checkAdjustTroveManagers(_troveId, _collChange, _isCollIncrease, _feUSDChange, _isDebtIncrease);

        // Set initial balances to make sure there are not lefovers
        _setInitialTokensAndBalances(collToken, feUSDToken, _initialBalances);
    ...
    function closeTroveToRawHYPE(uint256 _troveId) external {
        ...
        WHYPE.withdraw(ETH_GAS_COMPENSATION);
    ...
    function receiveFlashLoanOnCloseTroveFromCollateral(
        ...
        //require(receivedfeUSDAmount >= trove.entireDebt, "GCZ: Not enough feUSD obtained to repay");
