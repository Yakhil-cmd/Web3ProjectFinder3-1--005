...
contract LeverageLSTZapper is GasCompZapper, ILeverageZapper {
    ...
    function initialize(
        ...
        IExchange _exchange
    ) external override(GasCompZapper, ILeverageZapper) initializer {
        __GasCompZapper_init(_addressesRegistry, _flashLoanProvider, _exchange);
        // Approval of WHYPE and Coll to BorrowerOperations is done in parent GasCompZapper
        // Approve feUSD to exchange module (Coll is approved in parent GasCompZapper)
    ...
    function receiveFlashLoanOnOpenLeveragedTrove(
        ...
        // We compute feUSDAmount off-chain for efficiency
    ...
    function leverUpTrove(LeverUpTroveParams calldata _params) external {
        ...
        // Set initial balances to make sure there are not lefovers
        InitialBalances memory initialBalances;
        _setInitialTokensBalancesAndReceiver(collToken, feUSDToken, initialBalances, receiver);
