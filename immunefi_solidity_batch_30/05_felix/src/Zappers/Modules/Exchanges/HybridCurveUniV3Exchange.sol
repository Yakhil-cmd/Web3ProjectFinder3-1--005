...
contract HybridCurveUniV3Exchange is LeftoversSweep, IExchange {
    ...
    uint128 public immutable feUSD_TOKEN_INDEX;
    ...
    // feUSD -> USDC on Curve; then USDC -> WETH, and optionally WETH -> Coll, on UniV3
    function swapFromfeUSD(uint256 _feUSDAmount, uint256 _minCollAmount) external {
        ...
        feUSDToken.approve(address(curvePool), _feUSDAmount);

        uint256 curveUsdcAmount = curvePool.exchange(int128(feUSD_TOKEN_INDEX), int128(USDC_INDEX), _feUSDAmount, 0);
    ...
    // Optionally Coll -> WETH, and WETH -> USDC on UniV3; then USDC -> feUSD on Curve
    function swapTofeUSD(uint256 _collAmount, uint256 _minfeUSDAmount) external returns (uint256) {
        ...
        uint256 feUSDAmount =
            curvePool.exchange(int128(USDC_INDEX), int128(feUSD_TOKEN_INDEX), uniV3UsdcAmount, _minfeUSDAmount);
        feUSDToken.transfer(msg.sender, feUSDAmount);
    ...
    function _setHybridExchangeInitialBalances(InitialBalances memory initialBalances) internal view {
        initialBalances.tokens[0] = IERC20(address(feUSDToken));
