...
/*
 * The Stability Pool holds feUSD tokens deposited by Stability Pool depositors.
 *
 * When a trove is liquidated, then depending on system conditions, some of its feUSD debt gets offset with
 * feUSD in the Stability Pool:  that is, the offset debt evaporates, and an equal amount of feUSD tokens in the Stability Pool is burned.
 *
 * Thus, a liquidation causes each depositor to receive a feUSD loss, in proportion to their deposit as a share of total deposits.
 ...
 * of the total feUSD in the Stability Pool, depletes 40% of each deposit.
 *
 ...
 * --- MIN BOLD IN SP ---
 *
 * Once totalBoldDeposits has become >=1e18, a liquidation may never fully empty the Pool - a minimum of 1 BOLD remains in the SP at all times thereafter.
 ...
 *
 */
contract StabilityPool is Initializable, LiquityBase, IStabilityPool, IStabilityPoolEvents {
    ...
    // Tracker for feUSD held in the pool. Changes when users deposit/withdraw, and when Trove debt is offset.
    uint256 internal totalfeUSDDeposits;

    // Total remaining feUSD yield gains (from Trove interest mints) held by SP, and not yet paid out to depositors
    // From the contract's perspective, this is a write-only variable.
    uint256 internal yieldGainsOwed;
    // Total remaining feUSD yield gains (from Trove interest mints) held by SP, not yet paid out to depositors,
    ...
    struct Snapshots {
        ...
        uint256 P;
        uint256 B; // feUSD reward sum from minted interest
        uint256 scale;
    }
    ...
    * after a series of liquidations have occurred, each of which cancel some feUSD debt with the deposit.
    *
    ...
    */
    ...
    event PriceFeedChanged(address _newPriceFeed);
    ...
    function getTotalfeUSDDeposits() external view override returns (uint256) {
        return totalfeUSDDeposits;
    }
    ...
    */
    function provideToSP(uint256 _topUp, bool _doClaim) external override {
        ...
        uint256 compoundedfeUSDDeposit = getCompoundedfeUSDDeposit(msg.sender);
        (uint256 keptYieldGain, uint256 yieldGainToSend) = _getYieldToKeepOrSend(currentYieldGain, _doClaim);
        uint256 newDeposit = compoundedfeUSDDeposit + _topUp + keptYieldGain;
        ...
        emit DepositOperation(
            ...
            initialDeposit - compoundedfeUSDDeposit,
            int256(_topUp),
            currentYieldGain,
            yieldGainToSend,
            currentCollGain,
        ...
        feUSDToken.sendToPool(msg.sender, address(this), _topUp);
        _updateTotalfeUSDDeposits(_topUp + keptYieldGain, 0);
        _decreaseYieldGainsOwed(currentYieldGain);
        _sendfeUSDToDepositor(msg.sender, yieldGainToSend);
    ...
    function provideToSpOnBehalfOf(address _onBehalfOf, uint256 _topUp) external {
        _requireNonZeroAmount(_topUp);

        activePool.mintAggInterest();

        uint256 initialDeposit = deposits[_onBehalfOf].initialValue;

        uint256 currentCollGain = getDepositorCollGain(_onBehalfOf);
        uint256 currentYieldGain = getDepositorYieldGain(_onBehalfOf);
        uint256 compoundedfeUSDDeposit = getCompoundedfeUSDDeposit(_onBehalfOf);
        (uint256 keptYieldGain, uint256 yieldGainToSend) = _getYieldToKeepOrSend(currentYieldGain, false);
        uint256 newDeposit = compoundedfeUSDDeposit + _topUp + keptYieldGain;
        (uint256 newStashedColl, uint256 collToSend) =
            _getNewStashedCollAndCollToSend(_onBehalfOf, currentCollGain, false);

        emit DepositOperationOnBehalfOf(
            msg.sender,
            _onBehalfOf,
            Operation.provideToSP,
            initialDeposit - compoundedfeUSDDeposit,
        ...
        _updateDepositAndSnapshots(_onBehalfOf, newDeposit, newStashedColl);
        feUSDToken.sendToPool(msg.sender, address(this), _topUp);
        _updateTotalfeUSDDeposits(_topUp + keptYieldGain, 0);
        _decreaseYieldGainsOwed(currentYieldGain);
        _sendfeUSDToDepositor(_onBehalfOf, yieldGainToSend);
        _sendCollGainToDepositor(collToSend);
    ...
    */
    function withdrawFromSP(uint256 _amount, bool _doClaim) external override {
        ...
        uint256 currentYieldGain = getDepositorYieldGain(msg.sender);
        uint256 compoundedfeUSDDeposit = getCompoundedfeUSDDeposit(msg.sender);
        uint256 feUSDToWithdraw = LiquityMath._min(_amount, compoundedfeUSDDeposit);
        (uint256 keptYieldGain, uint256 yieldGainToSend) = _getYieldToKeepOrSend(currentYieldGain, _doClaim);
        uint256 newDeposit = compoundedfeUSDDeposit - feUSDToWithdraw + keptYieldGain;
        (uint256 newStashedColl, uint256 collToSend) =
            _getNewStashedCollAndCollToSend(msg.sender, currentCollGain, _doClaim);
        ...
        _updateDepositAndSnapshots(msg.sender, newDeposit, newStashedColl);
        _decreaseYieldGainsOwed(currentYieldGain);
        uint256 newTotalFeUSDDeposits = _updateTotalfeUSDDeposits(keptYieldGain, feUSDToWithdraw);
        _sendfeUSDToDepositor(msg.sender, feUSDToWithdraw + yieldGainToSend);
        _sendCollGainToDepositor(collToSend);

        require(newTotalFeUSDDeposits >= MIN_FEUSD_IN_SP, "Withdrawal must leave totalFeUSDDeposits >= 1e18");
    }
    ...
    function triggerfeUSDRewards(uint256 _feUSDYield) external {
        ...
        _updateYieldRewardsSum(_feUSDYield);
    }
    ...
    function _updateYieldRewardsSum(uint256 _newYield) internal {
        ...
        if (totalfeUSDDeposits < MIN_FEUSD_IN_SP) {
        ...
        scaleToB[currentScale] += P * accumulatedYieldGains / totalfeUSDDeposits;
    ...
    */
    function offset(uint256 _debtToOffset, uint256 _collToAdd) external override {
       _requireCallerIsTroveManager();
        ...
        scaleToS[currentScale] += P * _collToAdd / totalfeUSDDeposits;
        emit S_Updated(scaleToS[currentScale], currentScale);

        uint256 numerator = P * (totalfeUSDDeposits - _debtToOffset);
        uint256 newP = numerator / totalfeUSDDeposits;

        // For `P` to turn zero, `totalBoldDeposits` has to be greater than `P * (totalBoldDeposits - _debtToOffset)`.
        // The minimum value of `totalBoldDeposits - _debtToOffset` is `1e18`, and it can be shown that `P` is always in
        // range (1e27, 1e36]. Thus, to turn `P` zero, `totalBoldDeposits` has to be greater than `(1e27 + 1) * 1e18`,
        ...
        while (newP < P_PRECISION / SCALE_FACTOR) {
            ...
            newP = numerator / totalfeUSDDeposits;
            currentScale += 1;
    ...
    function _moveOffsetCollAndDebt(uint256 _collToAdd, uint256 _debtToOffset) internal {
        ...
        _updateTotalfeUSDDeposits(0, _debtToOffset);

        // Burn the debt that was successfully offset
        feUSDToken.burn(address(this), _debtToOffset);
    ...
    function _updateTotalfeUSDDeposits(uint256 _depositIncrease, uint256 _depositDecrease) internal returns (uint256) {
        if (_depositIncrease == 0 && _depositDecrease == 0) return totalfeUSDDeposits;
        uint256 newTotalfeUSDDeposits = totalfeUSDDeposits + _depositIncrease - _depositDecrease;
        totalfeUSDDeposits = newTotalfeUSDDeposits;

        emit StabilityPoolfeUSDBalanceUpdated(newTotalfeUSDDeposits);
        return newTotalfeUSDDeposits;
    }
    ...
    function getDepositorYieldGainWithPending(address _depositor) external view override returns (uint256) {
        if (totalfeUSDDeposits < MIN_FEUSD_IN_SP) return 0;
        ...
        if (currentScale <= snapshots.scale + SCALE_SPAN) {
            normalizedGains += P * pendingSPYield / totalfeUSDDeposits / SCALE_FACTOR ** (currentScale - snapshots.scale);
        }
    ...
    function getCompoundedfeUSDDeposit(address _depositor) public view override returns (uint256 compoundedDeposit) {
        uint256 initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) return 0;
    ...
    function _sendfeUSDToDepositor(address _depositor, uint256 _feUSDToSend) internal {
        if (_feUSDToSend == 0) return;
        feUSDToken.returnFromPool(address(this), _depositor, _feUSDToSend);
    }
    ...
    function _requireCallerIsAdmin() internal view {
        ICollateralRegistry collRegistry = troveManager.collateralRegistry();
        require(msg.sender == collRegistry.owner(), "StabilityPool: Caller is not owner");
    }
