...
interface IBorrowerOperations is IAddRemoveManagers {
    ...
    function withdrawfeUSD(uint256 _troveId, uint256 _amount, uint256 _maxUpfrontFee) external;

    function repayfeUSD(uint256 _troveId, uint256 _amount) external;
    ...
    function maxDebtCap() external view returns (uint256);

    function isTemporaryShutdown() external view returns(bool);
