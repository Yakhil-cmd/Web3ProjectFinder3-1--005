...
import "./Interfaces/IfeUSDRewardsReceiver.sol";
import "./Interfaces/ICollateralRegistry.sol";

/*
 * The Active Pool holds the collateral and feUSD debt (but not feUSD tokens) for all active troves.
 *
 * When a trove is liquidated, it's Coll and feUSD debt are transferred from the Active Pool, to either the
 * Stability Pool, the Default Pool, or both, depending on the liquidation conditions.
 *
 */
contract ActivePool is Initializable, IActivePool { // Ownable can be removed because we are relying on the owner set on the collateral registry
    ...
    IfeUSDRewardsReceiver public stabilityPool;
    ...
    */
    ...
    // The fraction of the interest minted by the Active Pool that goes to the Stability Pool
    ...
    event ActivePoolfeUSDDebtUpdated(uint256 _recordedDebtSum);
    event ActivePoolCollBalanceUpdated(uint256 _collBalance);
    event SPYieldChanged(uint256 _SPYieldSplit);
    event InterestRouterChanged(address _newInterestRouter);
    ...
    function initialize(
        ...
        stabilityPool = IfeUSDRewardsReceiver(_addressesRegistry.stabilityPool());
    ...
    */
        ...
        returns (uint256)
    {
        ...
        newAggRecordedDebt += _troveChange.appliedRedistfeUSDDebtGain;
        ...
        // Avoid division by 0 if the first ever borrower tries to borrow 0 feUSD
        // Borrowing 0 feUSD is not allowed, but our check of debt >= MIN_DEBT happens _after_ calculating the upfront
    ...
    function setSPYieldSplit(uint256 _SPYieldSplit) external {
        ...
        _mintAggInterest(0); // Clear pending interest before changing the split to avoid minting incorrect amounts 
        SP_YIELD_SPLIT = _SPYieldSplit;
        emit SPYieldChanged(_SPYieldSplit);
    }

    function setInterestRouter(address _interestRouter) external {
        _requireCallerIsAdmin();
        _mintAggInterest(0); // Clear pending interest before changing the interest router to avoid minting interest to the wrong address
        interestRouter = IInterestRouter(_interestRouter);
        emit InterestRouterChanged(_interestRouter);
    }
    ...
    function _mintAggInterest(uint256 _upfrontFee) internal returns (uint256 mintedAmount) {
        ...
        // Mint part of the feUSD interest to the SP and part to the router for LPs.
        if (mintedAmount > 0) {
            uint256 spYield = SP_YIELD_SPLIT * mintedAmount / DECIMAL_PRECISION;
            uint256 remainderToLPs = mintedAmount - spYield;

            feUSDToken.mint(address(interestRouter), remainderToLPs);

            if (spYield > 0) {
                feUSDToken.mint(address(stabilityPool), spYield);
                stabilityPool.triggerfeUSDRewards(spYield);
            }
        }
        ...
        internal
    {
        ...
        if (_troveChange.batchAccruedManagementFee > 0) {
            feUSDToken.mint(_batchAddress, _troveChange.batchAccruedManagementFee);
        }
    ...
    function _requireCallerIsAdmin() internal view {
        ICollateralRegistry collRegistry = ITroveManager(troveManagerAddress).collateralRegistry();
        require(msg.sender == collRegistry.owner(), "ActivePool: Caller is not Admin");
    }
}
