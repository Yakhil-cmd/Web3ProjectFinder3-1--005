...
import "./Dependencies/LiquityBaseLst.sol";
import "./Libraries/TroveManagerInit.sol";
import "./Dependencies/Constants.sol";


contract TroveManagerLst is Initializable, LiquityBaseLst, ITroveManager, ITroveEvents {
    ...
    IfeUSDToken internal feUSDToken;
    ...
    /*
    * L_coll and L_feUSDDebt track the sums of accumulated liquidation rewards per unit staked. During its lifetime, each stake earns:
    *
    * An Coll gain of ( stake * [L_coll - L_coll(0)] )
    * A feUSDDebt increase  of ( stake * [L_feUSDDebt - L_feUSDDebt(0)] )
    *
    * Where L_coll(0) and L_feUSDDebt(0) are snapshots of L_coll and L_feUSDDebt for the active Trove taken at the instant the stake was made
    */
    uint256 internal L_coll;
    uint256 internal L_feUSDDebt;

    // Map active troves to their RewardSnapshot
    mapping(uint256 => RewardSnapshot) public rewardSnapshots;

    // Object containing the Coll and feUSD snapshots for a given active trove
    ...
    uint256 internal lastfeUSDDebtError_Redistribution;
    ...
    **/
    ...
    struct SingleRedemptionValues {
        ...
        uint256 collLot;
        uint256 collFee;
        uint256 appliedRedistfeUSDDebtGain;
        uint256 oldWeightedRecordedDebt;
        uint256 newWeightedRecordedDebt;
        uint256 newStake;
        bool isZombieTrove;
    ...
    event NewMCRSet(uint256 _cmr);
    event NewCCRSet(uint256 _ccr);
    event NewPriceFeed(address _priceFeed);
    ...
    function initialize(IAddressesRegistry _addressesRegistry) external initializer {
        TroveManagerInit.initialize(_addressesRegistry);
    }
    ...
    function setMCR(uint256 _newMCR) external {
        if(msg.sender != collateralRegistry.owner()) revert NotOwner();
        MCR = _newMCR;
        emit NewMCRSet(_newMCR);
    }
    
    function setCCR(uint256 _newCCR) external {
        if(msg.sender != collateralRegistry.owner()) revert NotOwner();
        CCR = _newCCR;
        emit NewCCRSet(_newCCR);
    }

    function setPriceFeed(address _priceFeed) external {
        if(msg.sender != collateralRegistry.owner()) revert NotOwner();
        priceFeed = IPriceFeedLst(_priceFeed);
    ...
    function _liquidate(
        ...
        _movePendingTroveRewardsToActivePool(_defaultPool, trove.redistfeUSDDebtGain, trove.redistCollGain);
        ...
        ) = _getOffsetAndRedistributionVals(trove.entireDebt, collToLiquidate, _feUSDInStabPool, _price);
        ...
        troveChange.appliedRedistfeUSDDebtGain = trove.redistfeUSDDebtGain;
        _closeTrove(
            _troveId,
            troveChange,
            batchAddress,
        ...
        if (isTroveInBatch) {
            singleLiquidation.oldWeightedRecordedDebt =
                batch.weightedRecordedDebt + (trove.entireDebt - trove.redistfeUSDDebtGain) * batch.annualInterestRate;
            ...
            troveChange.oldWeightedRecordedBatchManagementFee = batch.weightedRecordedBatchManagementFee
                + (trove.entireDebt - trove.redistfeUSDDebtGain) * batch.annualManagementFee;
        ...
        emit TroveOperation({
            ...
            _annualInterestRate: 0,
            _debtIncreaseFromRedist: trove.redistfeUSDDebtGain,
    ...
    function _getCollGasCompensation(uint256 _entireColl) internal pure returns (uint256 compensation) {
        // return _entireColl / COLL_GAS_COMPENSATION_DIVISOR;

        uint256 _collGasCompensationDivisor = COLL_GAS_COMPENSATION_DIVISOR;
        assembly {
            compensation := div(_entireColl, _collGasCompensationDivisor)
        }
    }
    ...
    */
        ...
        )
    {
         ...
         *
         *  If the trove's debt is larger than the deposited feUSD in the Stability Pool:
         *
         *  - Offset an amount of the trove's debt equal to the feUSD in the Stability Pool
         *  - Send a fraction of the trove's collateral to the Stability Pool, equal to the fraction of its offset debt
         *
         */
        if (_feUSDInStabPool > 0) {
            debtToOffset = LiquityMath._min(_entireTroveDebt, _feUSDInStabPool);
    ...
    function batchLiquidateTroves(uint256[] memory _troveArray) public override {
        ...
        uint256 totalfeUSDDeposits = stabilityPoolCached.getTotalfeUSDDeposits();
        uint256 feUSDToLeaveInSP = LiquityMath._min(MIN_FEUSD_IN_SP, totalfeUSDDeposits);
        uint256 feUSDInSPForOffsets = totalfeUSDDeposits - feUSDToLeaveInSP;

        // Perform the appropriate liquidation sequence - tally values and obtain their totals.
        _batchLiquidateTroves(defaultPoolCached, price, feUSDInSPForOffsets, _troveArray, totals, troveChange);
        ...
        // Move liquidated Coll and feUSD to the appropriate pools
    ...
    function _batchLiquidateTroves(
        IDefaultPool _defaultPool,
        uint256 _price,
        uint256 _feUSDInSPForOffsets,
        uint256[] memory _troveArray,
        LiquidationValues memory totals,
        TroveChange memory troveChange
    ) internal {
        uint256 remainingfeUSDInSPForOffsets = _feUSDInSPForOffsets;
        ...
        for (uint256 i = 0; i < _troveArray.length; i++) {
            ...
            if (ICR < MCR) {
                ...
                _liquidate(_defaultPool, troveId, remainingfeUSDInSPForOffsets, _price, trove, singleLiquidation);
                remainingfeUSDInSPForOffsets -= singleLiquidation.debtToOffset;
    ...
    function _addLiquidationValuesToTotals(
        ...
        troveChange.appliedRedistfeUSDDebtGain += _trove.redistfeUSDDebtGain;
    ...
    function _sendGasCompensation(IActivePool _activePool, address _liquidator, uint256 _eth, uint256 _coll) internal {
        if (_eth > 0) {
            WHYPE.transferFrom(gasPoolAddress, _liquidator, _eth);
        }
    ...
    function _movePendingTroveRewardsToActivePool(IDefaultPool _defaultPool, uint256 _feUSD, uint256 _coll) internal {
        if (_feUSD > 0) {
            _defaultPool.decreasefeUSDDebt(_feUSD);
        }
    ...
    function _applySingleRedemption(
        ...
        // Decrease the debt and collateral of the current Trove according to the feUSD lot and corresponding ETH to send
        uint256 newDebt = _singleRedemption.trove.entireDebt - _singleRedemption.feUSDLot;
        uint256 newColl = _singleRedemption.trove.entireColl - _singleRedemption.collLot;

        _singleRedemption.appliedRedistfeUSDDebtGain = _singleRedemption.trove.redistfeUSDDebtGain;

        if (_isTroveInBatch) {
            _getLatestBatchData(_singleRedemption.batchAddress, _singleRedemption.batch);
            // We know feUSDLot <= trove entire debt, so this subtraction is safe
            uint256 newAmountForWeightedDebt = _singleRedemption.batch.entireDebtWithoutRedistribution
                + _singleRedemption.trove.redistfeUSDDebtGain - _singleRedemption.feUSDLot;
            ...
            TroveChange memory troveChange;
            troveChange.debtDecrease = _singleRedemption.feUSDLot;
            troveChange.collDecrease = _singleRedemption.collLot;
            troveChange.appliedRedistfeUSDDebtGain = _singleRedemption.trove.redistfeUSDDebtGain;
            ...
            // This call could revert due to BatchSharesRatioTooHigh if trove.redistCollGain > feUSDLot
        ...
        _movePendingTroveRewardsToActivePool(
            _defaultPool, _singleRedemption.trove.redistfeUSDDebtGain, _singleRedemption.trove.redistCollGain
        );
        ...
        if (_isTroveInBatch) {
            emit BatchedTroveUpdated({
                ...
                _snapshotOfTotalDebtRedist: L_feUSDDebt
            });
        } else {
            emit TroveUpdated({
                _troveId: _singleRedemption.troveId,
        ...
        emit TroveOperation({
            ...
            _annualInterestRate: _singleRedemption.trove.annualInterestRate,
            _debtIncreaseFromRedist: _singleRedemption.trove.redistfeUSDDebtGain,
            _debtIncreaseFromUpfrontFee: 0,
            _debtChangeFromOperation: -int256(_singleRedemption.feUSDLot),
    ...
    // Redeem as much collateral as possible from _borrower's Trove in exchange for feUSD up to _maxfeUSDamount
    function _redeemCollateralFromTrove(
        IDefaultPool _defaultPool,
        SingleRedemptionValues memory _singleRedemption,
        uint256 _maxfeUSDamount,
        uint256 _price,
        ...
        _singleRedemption.feUSDLot = LiquityMath._min(_maxfeUSDamount, _singleRedemption.trove.entireDebt);

        // Get the amount of Coll equal in USD value to the feUSDLot redeemed
        uint256 correspondingColl = _singleRedemption.feUSDLot * DECIMAL_PRECISION / _price;
        ...
        // newDebt >= MIN_DEBT thanks to feUSD debt redistribution, which means it _could_ be made active again,
    ...
    /* Send _feUSDamount feUSD to the system and redeem the corresponding amount of collateral from as many Troves as are needed to fill the redemption
    ...
    */
    function redeemCollateral(
        ...
        uint256 remainingfeUSD = _feUSDamount;
        ...
        // Loop through the Troves starting from the one with lowest interest rate until _amount of feUSD is exchanged for collateral
        if (_maxIterations == 0) _maxIterations = type(uint256).max;
        while (singleRedemption.troveId != 0 && remainingfeUSD > 0 && _maxIterations > 0) {
            _maxIterations--;
            ...
            _redeemCollateralFromTrove(defaultPool, singleRedemption, remainingfeUSD, _price, _redemptionRate);

            totalsTroveChange.collDecrease += singleRedemption.collLot;
            totalsTroveChange.debtDecrease += singleRedemption.feUSDLot;
            totalsTroveChange.appliedRedistfeUSDDebtGain += singleRedemption.appliedRedistfeUSDDebtGain;
            ...
            remainingfeUSD -= singleRedemption.feUSDLot;
            singleRedemption.troveId = nextUserToCheck;
            singleRedemption.isZombieTrove = false;
        }
        ...
        emit Redemption(
            _feUSDamount, totalsTroveChange.debtDecrease, totalsTroveChange.collDecrease, totalCollFee, _price
        );
        ...
        // We’ll burn all the feUSD together out in the CollateralRegistry, to save gas
    ...
    function _urgentRedeemCollateralFromTrove(
        ...
        // Get the amount of ETH equal in USD value to the feUSD lot redeemed
        _singleRedemption.collLot = _singleRedemption.feUSDLot * (DECIMAL_PRECISION + URGENT_REDEMPTION_BONUS) / _price;
    ...
    function urgentRedemption(uint256 _feUSDAmount, uint256[] calldata _troveIds, uint256 _minCollateral) external {
        _requireIsShutDown();
        _requireAmountGreaterThanZero(_feUSDAmount);
        _requirefeUSDBalanceCoversRedemption(feUSDToken, msg.sender, _feUSDAmount);
        ...
        for (uint256 i = 0; i < _troveIds.length; i++) {
            ...
            _urgentRedeemCollateralFromTrove(defaultPool, remainingfeUSD, price, singleRedemption);
        ...
        emit Redemption(_feUSDAmount, totalsTroveChange.debtDecrease, totalsTroveChange.collDecrease, 0, price);
        ...
        // Burn feUSD
        feUSDToken.burn(msg.sender, totalsTroveChange.debtDecrease);
    }
    ...
    function shutdown() external {
        ...
        assembly {
            sstore(shutdownTime.slot, timestamp())
            mstore(0x00, 0x6d8f962d) // setShutdownFlag()
            let success := call(gas(), shr(16, sload(activePool.slot)), 0x00, 28, 32, 0x00, 0x20) // gas, address, value, offset, size, returnDataOffset, returnDataSize
            if iszero(success) {
                revert(0, 0)
            }
        }
    ...
    function _updateTroveRewardSnapshots(uint256 _troveId) internal {
        ...
        rewardSnapshots[_troveId].feUSDDebt = L_feUSDDebt;
    }
    ...
    function _getLatestTroveData(uint256 _troveId, LatestTroveData memory trove) internal view {
        ...
        trove.redistfeUSDDebtGain = stake * (L_feUSDDebt - rewardSnapshots[_troveId].feUSDDebt) / DECIMAL_PRECISION;
        ...
        trove.entireDebt = trove.recordedDebt + trove.redistfeUSDDebtGain + trove.accruedInterest;
    ...
    function _getLatestTroveDataFromBatch(
        ...
        _latestTroveData.redistfeUSDDebtGain =
            stake * (L_feUSDDebt - rewardSnapshots[_troveId].feUSDDebt) / DECIMAL_PRECISION;
        ...
        _latestTroveData.entireDebt = _latestTroveData.recordedDebt + _latestTroveData.redistfeUSDDebtGain
    ...
    function _redistributeDebtAndColl(
        ...
        * error correction, to keep the cumulative error low in the running totals L_coll and L_feUSDDebt:
        *
        ...
        */
        ...
        uint256 feUSDDebtNumerator = _debtToRedistribute * DECIMAL_PRECISION + lastfeUSDDebtError_Redistribution;

        // Get the per-unit-staked terms
        uint256 collRewardPerUnitStaked = collNumerator / totalStakes;
        uint256 feUSDDebtRewardPerUnitStaked = feUSDDebtNumerator / totalStakes;

        lastCollError_Redistribution = collNumerator - collRewardPerUnitStaked * totalStakes;
        lastfeUSDDebtError_Redistribution = feUSDDebtNumerator - feUSDDebtRewardPerUnitStaked * totalStakes;

        // Add per-unit-staked terms to the running totals
        L_coll = L_coll + collRewardPerUnitStaked;
        L_feUSDDebt = L_feUSDDebt + feUSDDebtRewardPerUnitStaked;

        _defaultPool.increasefeUSDDebt(_debtToRedistribute);
    ...
    */
    ...
    function _getInterestPeriod(uint256 _lastDebtUpdateTime) internal view returns (uint256 interestPeriod) {
        if (shutdownTime == 0) {
...
/*         if (msg.sender != address(borrowerOperations)) {
        ...
        assembly {
            if iszero(eq(caller(), sload(borrowerOperations.slot))) {
                mstore(0x00, 0x558b0d1f)
                revert(0x1c, 0x04)
            }
        }
    }
...
/*         if (msg.sender != address(collateralRegistry)) {
        ...
        assembly {
            if iszero(eq(caller(), sload(collateralRegistry.slot))) {
                mstore(0x00, 0x049908ac)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _requireMoreThanOneTroveInSystem(uint256 TroveIdsArrayLength) internal pure {
        assembly {
            if or(eq(TroveIdsArrayLength, 1), lt(TroveIdsArrayLength, 1)) {
                mstore(0x00, 0x3307cee3)
                revert(0x1c, 0x04)
            }
        }
        // if(TroveIdsArrayLength <= 1) revert OnlyOneTroveLeft(); 
    }

    function _requireIsShutDown() internal view {

        assembly {
            if iszero(sload(shutdownTime.slot)) {
                mstore(0x00, 0xf8b7c7bf)
                revert(0x1c, 0x04)
            }
        }
        // if(shutdownTime == 0) revert NotShutDown();
    }
    ...
    function _requireAmountGreaterThanZero(uint256 _amount) internal pure {
        ...
        // if(_amount == 0) revert ZeroAmount(); // tweak this for invariants test
    }

    function _requirefeUSDBalanceCoversRedemption(IfeUSDToken _feUSDToken, address _redeemer, uint256 _amount)
        internal
        view
    {
        assembly {
            mstore(0x00, 0x70a08231)
            mstore(0x20, _redeemer)
            let success := staticcall(gas(), _feUSDToken, 28, 64, 0x00, 0x20)
            if iszero(success) {
                revert(0, 0)
            }
            if lt(mload(0x00), _amount) {
                mstore(0x00, 0x0f4a26fe)
                revert(0x1c, 0x04)
            }
        }
        // NotEnoughfeUSDBalance
    }
    ...
    function getUnbackedPortionPriceAndRedeemability() external returns (uint256, uint256, bool) {
        ...
        uint256 spSize = stabilityPool.getTotalfeUSDDeposits();
    ...
    function setTroveStatusToActive(uint256 _troveId) external {
        ...
        // }
        assembly {
            if eq(sload(lastZombieTroveId.slot), _troveId) {
                sstore(lastZombieTroveId.slot, 0)
            }
        }
    }
    ...
    function onAdjustTroveInterestRate(
        ...
        _movePendingTroveRewardsToActivePool(
            defaultPool, _troveChange.appliedRedistfeUSDDebtGain, _troveChange.appliedRedistCollGain
        );
        ...
        emit TroveOperation({
            ...
            _annualInterestRate: _newAnnualInterestRate,
            _debtIncreaseFromRedist: _troveChange.appliedRedistfeUSDDebtGain,
            _debtIncreaseFromUpfrontFee: _troveChange.upfrontFee,
    ...
    function onSetInterestBatchManager(OnSetInterestBatchManagerParams calldata _params) external {
        ...
        _troveChange.debtIncrease = _params.troveDebt - _troveChange.appliedRedistfeUSDDebtGain - _troveChange.upfrontFee;
    ...
    function _updateBatchShares(
        ...
        uint256 debtIncrease =
            _troveChange.debtIncrease + _troveChange.upfrontFee + _troveChange.appliedRedistfeUSDDebtGain;
    ...
    function _requireBelowMaxSharesRatio(
        ...
        // }
        uint256 _MAX_BATCH_SHARES_RATIO = MAX_BATCH_SHARES_RATIO;
        assembly {
            if and(lt(mul(_currentBatchDebtShares, _MAX_BATCH_SHARES_RATIO), _batchDebt), _checkBatchSharesRatio) {
                mstore(0x00, 0x43593ae8)
                revert(0x1c, 0x04)
            }
        }
    }
    ...
    function _removeTroveSharesFromBatch(
        ...
        uint256 batchDebtDecrease = _newTroveDebt - _troveChange.upfrontFee - _troveChange.appliedRedistfeUSDDebtGain;
