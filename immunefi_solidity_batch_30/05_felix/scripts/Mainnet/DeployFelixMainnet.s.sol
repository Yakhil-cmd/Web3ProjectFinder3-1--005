...
import {ICryptoSwapFactory} from "../../src/Zappers/Modules/Exchanges/Curve/ICryptoSwapFactory.sol";
...
contract DeployFelixMainnet is Script, MetadataDeployment {
    using Strings for *;
    ...
    // Curve Info
    bytes32 public constant GAUGE_SALT = keccak256("curve.gauge.feUSD.WHYPE");

    /// @notice This address comes from https://github.com/curvefi/curve-core/blob/main/deployments/prod/hyperliquid.yaml
    ICryptoSwapFactory public constant CURVE_CRYPTO_SWAP_FACTORY =
        ICryptoSwapFactory(0xc9Fe0C63Af9A39402e8a5514f9c43Af0322b665F);
    /// @notice This address comes from: https://github.com/curvefi/curve-core/blob/main/deployments/prod/hyperliquid.yaml
    ...
    // Curve Pool Infos
    string public constant POOL_NAME = "feUSD/WHYPE";
    string public constant POOL_SYMBOL = "feUSDWHYPE";
    ...
    uint256 public constant A = 2700000;
    uint256 public constant GAMMA = 1300000000000;
    uint256 public constant MID_FEE = 2999999;
    uint256 public constant OUT_FEE = 80000000;
    uint256 public constant FEE_GAMMA = 350000000000000;
    uint256 public constant ALLOWED_EXTRA_PROFIT = 100000000000;
    uint256 public constant ADJUSTMENT_STEP = 100000000000;
    uint256 public constant MA_HALF_TIME = 600;
    uint256 public constant INITIAL_PRICE = 15 ether; // This is coin1 / coin0 (expressed in 18 decimals) TODO: adapt it at moment of deployment
    ...
    uint256 public constant UPDATED_AT_TIME_BUFFER = 1 minutes;
    ...
    // Branch contracts
    mapping(Collaterals => mapping(ContractTypes => address))
        public branchContractAddresses;
    ...
    function _deployContractImplementation() internal {
        contractImplementationAddresses[ContractTypes.ACTIVE_POOL] = address(
            new ActivePool()
        );
        contractImplementationAddresses[
            ContractTypes.ADDRESSES_REGISTRY
        ] = address(new AddressesRegistry());
        contractImplementationAddresses[
            ContractTypes.ADMIN_CONTROLLER
        ] = address(new AdminController());
        contractImplementationAddresses[ContractTypes.FEUSD_TOKEN] = address(
            new feUSDToken()
        );
        contractImplementationAddresses[
            ContractTypes.BORROWER_OPERATIONS
        ] = address(new BorrowerOperations());
        contractImplementationAddresses[
            ContractTypes.COLLATERAL_REGISTRY
        ] = address(new CollateralRegistry());
        contractImplementationAddresses[
            ContractTypes.COLL_SURPLUS_POOL
        ] = address(new CollSurplusPool());
        contractImplementationAddresses[ContractTypes.DEFAULT_POOL] = address(
            new DefaultPool()
        );
        contractImplementationAddresses[ContractTypes.GAS_POOL] = address(
            new GasPool()
        );
        contractImplementationAddresses[ContractTypes.HINT_HELPERS] = address(
            new HintHelpers()
        );
        contractImplementationAddresses[
            ContractTypes.MULTI_TROVE_GETTER
        ] = address(new MultiTroveGetter());
        contractImplementationAddresses[
            ContractTypes.PRICE_FEEDS
        ] = _deployCorrectPriceFeedImplementation();
        contractImplementationAddresses[ContractTypes.SORTED_TROVES] = address(
            new SortedTroves()
        );
        contractImplementationAddresses[ContractTypes.STABILITY_POOL] = address(
            new StabilityPool()
        );
        contractImplementationAddresses[ContractTypes.TROVE_MANAGER] = address(
            new TroveManager()
        );
        contractImplementationAddresses[ContractTypes.TROVE_NFT] = address(
            new TroveNFT()
        );
        contractImplementationAddresses[
            ContractTypes.INTEREST_ROUTER
        ] = address(new InterestRouterV2());
        contractImplementationAddresses[ContractTypes.METADATA_NFT] = address(
            new MetadataNFT()
        );
    }

    function _deployProxyAdmins() internal {
        proxyAdminAddresses.proxyAdminForCoreContracts = address(
            new ProxyAdmin()
        );
        proxyAdminAddresses.proxyAdminForAdminController = address(
            new ProxyAdmin()
        );
    }

    function _deployfeUSDToken() internal {
        bytes memory _initDataForfeUSDToken = abi.encodeWithSelector(
            feUSDToken.initialize.selector,
    ...
    function _deployCollateralRegistry() internal {
        IfeUSDToken _feUSDToken = IfeUSDToken(
            singletonContractAddresses.feUSDToken
        );
        IERC20Metadata[] memory _tokens = _buildCollateralsArray();
        ITroveManager[]
            memory _troveManagers = _buildTroveManagerAddressesArray();
    ...
    function _deployHintHelpers() internal {
        bytes memory _initDataForHintHelpers = abi.encodeWithSelector(
            HintHelpers.initialize.selector,
            singletonContractAddresses.collateralRegistry
        );
    ...
    function _deployMultiTroveGetter() internal {
        bytes memory _initDataForMultiTroveGetter = abi.encodeWithSelector(
            MultiTroveGetter.initialize.selector,
            singletonContractAddresses.collateralRegistry
        );
    ...
    function _deployMetadataNFT() internal {
        address _metadataNFTImpl = contractImplementationAddresses[
            ContractTypes.METADATA_NFT
        ];
        assert(_metadataNFTImpl != address(0));
        assert(proxyAdminAddresses.proxyAdminForCoreContracts != address(0));
        assert(SALT != bytes32(0));

        MetadataNFT _metadataNFT = deployMetadata(
            SALT,
            address(proxyAdminAddresses.proxyAdminForCoreContracts),
            _metadataNFTImpl
        );
        singletonContractAddresses.metadataNFT = address(_metadataNFT);
    }

    function _deployCorrectPriceFeedImplementation()
        internal
        returns (address)
    {
    ...
    function _readCollateralParams()
        internal
        view
        returns (CollateralParams[] memory _collateralParams)
    {
        string memory collateralParamsJson = vm.readFile(
            "trove-manager-params.json"
        );
        bytes memory data = vm.parseJson(collateralParamsJson, ".data");
        return abi.decode(data, (CollateralParams[]));
    }

    function _buildCollateralsArray()
        internal
        view
        returns (IERC20Metadata[] memory _collateralFaucets)
    {
        _collateralFaucets = new IERC20Metadata[](COLLATERALS_LENGTH);
        _collateralFaucets[0] = IERC20Metadata(
            collateralParams[Collaterals.WHYPE].collToken
        );
    ...
    function _buildTroveManagerAddressesArray()
        internal
        view
        returns (ITroveManager[] memory _troveManagers)
    {
        _troveManagers = new ITroveManager[](COLLATERALS_LENGTH);

        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            address _forecastedAddress = _forecastAddressForTroveManager(
                Collaterals(i)
            );
    ...
    function _deployAddressesRegistries() internal {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            CollateralParams memory _collateralParams = collateralParams[
                Collaterals(i)
            ];
            ...
            assert(
                proxyAdminAddresses.proxyAdminForCoreContracts != address(0)
            );

            branchContractAddresses[Collaterals(i)][
                ContractTypes.ADDRESSES_REGISTRY
            ] = address(
                new TransparentUpgradeableProxy(
                    contractImplementationAddresses[
    ...
    function _deployRedStonePriceFeeds() internal {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            ...
            address _borrowerOperations = _forecastAddressForBorrowerOperations(
                _collateral
            );
            CollateralParams memory _collateralParams = collateralParams[
                _collateral
            ...
            branchContractAddresses[_collateral][
                ContractTypes.PRICE_FEEDS
            ] = address(
                new TransparentUpgradeableProxy(
                    contractImplementationAddresses[ContractTypes.PRICE_FEEDS],
    ...
    function _deployInterestRouters() internal {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            IInterestRouterV2.AllocationConfig
                memory _allocationConfig = _createInterestRouterAllocationConfig();
            bytes memory _initDataForInterestRouter = abi.encodeWithSelector(
                IInterestRouterV2.initialize.selector,
                singletonContractAddresses.adminController,
                singletonContractAddresses.feUSDToken,
                _allocationConfig
            );
            assert(
    ...
    function _setCollateralRegistryForAdminControllerAndfeUSDToken() internal {
        address _collateralRegistry = singletonContractAddresses
            .collateralRegistry;
        assert(_collateralRegistry != address(0));
        IAdminController _adminController = IAdminController(
            singletonContractAddresses.adminController
        );
    ...
    function _setMultiSigAsDefaultAdminForAdminControllerAndRenounceRole()
        internal
    {
        AccessControlUpgradeable _adminController = AccessControlUpgradeable(
            singletonContractAddresses.adminController
        );
        _adminController.grantRole(
            _adminController.DEFAULT_ADMIN_ROLE(),
            multiSigWallet
        );
        _adminController.renounceRole(
            _adminController.DEFAULT_ADMIN_ROLE(),
            deployerInfo.deployerAddress
        );
        assert(
            _adminController.hasRole(
                _adminController.DEFAULT_ADMIN_ROLE(),
                multiSigWallet
            )
        );
    ...
    function _setAddressesRegistryForBranchContracts(
        Collaterals _collateral
    ) internal {
        address _addressesRegistry = branchContractAddresses[_collateral][
            ContractTypes.ADDRESSES_REGISTRY
        ];
        IAddressesRegistry.AddressVars memory _addressVars = IAddressesRegistry
            .AddressVars({
                collToken: IERC20Metadata(
                    collateralParams[_collateral].collToken
                ),
                borrowerOperations: IBorrowerOperations(
                    _forecastAddressForBorrowerOperations(_collateral)
                ),
                troveManager: ITroveManager(
                    _forecastAddressForTroveManager(_collateral)
                ),
                troveNFT: ITroveNFT(_forecastAddressForTroveNFT(_collateral)),
                metadataNFT: IMetadataNFT(
                    singletonContractAddresses.metadataNFT
                ),
                stabilityPool: IStabilityPool(
                    _forecastAddressForStabilityPool(_collateral)
                ),
                priceFeed: HLPriceFeed(
                    branchContractAddresses[_collateral][
                        ContractTypes.PRICE_FEEDS
                    ]
                ),
                activePool: IActivePool(
                    _forecastAddressForActivePool(_collateral)
                ),
                defaultPool: IDefaultPool(
                    _forecastAddressForDefaultPool(_collateral)
                ),
                gasPoolAddress: _forecastAddressForGasPool(_collateral),
                collSurplusPool: ICollSurplusPool(
                    _forecastAddressForCollSurplusPool(_collateral)
                ),
                sortedTroves: ISortedTroves(
                    _forecastAddressForSortedTroves(_collateral)
                ),
                ...
                hintHelpers: HintHelpers(
                    singletonContractAddresses.hintHelpers
                ),
                multiTroveGetter: MultiTroveGetter(
                    singletonContractAddresses.multiTroveGetter
                ),
                collateralRegistry: CollateralRegistry(
                    singletonContractAddresses.collateralRegistry
                ),
                feUSDToken: IfeUSDToken(singletonContractAddresses.feUSDToken),
                WHYPE: IWHYPE(collateralParams[Collaterals.WHYPE].collToken) // Needs to be the wrapped native token maybe we'll need to change interface
            });
    ...
    function _sanitizeAddressesRegistry(
        IAddressesRegistry.AddressVars memory _addressVars
    ) internal {
    ...
    function _setBranchAddressesForfeUSDToken() internal {
        ...
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            address _troveManager = branchContractAddresses[_collateral][
                ContractTypes.TROVE_MANAGER
            ];
            address _stabilityPool = branchContractAddresses[_collateral][
                ContractTypes.STABILITY_POOL
            ];
            address _borrowerOperations = branchContractAddresses[_collateral][
                ContractTypes.BORROWER_OPERATIONS
            ];
            address _activePool = branchContractAddresses[_collateral][
                ContractTypes.ACTIVE_POOL
            ];
    ...
    function _setAddressesRegistriesForAdminController() internal {
        ...
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            ...
            _adminController.addAddressRegistry(
    ...
    function _deployCoreContractsForOneBranch(
        Collaterals _collareral
    ) internal {
        address _addressesRegistry = branchContractAddresses[_collareral][
            ContractTypes.ADDRESSES_REGISTRY
        ];
    ...
    function _deployBorrowerOperations(
        address _addressesRegistry,
        Collaterals _collareral
    ) internal {
        branchContractAddresses[_collareral][
            ContractTypes.BORROWER_OPERATIONS
        ] = _deployProxy(
    ...
    function _deployTroveManager(
        ...
        branchContractAddresses[_collareral][
            ...
            abi.encodeWithSelector(
                TroveManager.initialize.selector,
                _addressesRegistry
            )
        );
        assert(
    ...
    function _deployTroveNFT(
        address _addressesRegistry,
        Collaterals _collareral
    ) internal {
        branchContractAddresses[_collareral][
            ContractTypes.TROVE_NFT
        ] = _deployProxy(
            contractImplementationAddresses[ContractTypes.TROVE_NFT],
            abi.encodeWithSelector(
                TroveNFT.initialize.selector,
                _addressesRegistry
            )
        );
        assert(
            branchContractAddresses[_collareral][ContractTypes.TROVE_NFT] ==
                _forecastAddressForTroveNFT(_collareral)
        );
    }
    ...
    function _deployStabilityPool(
        ...
        branchContractAddresses[_collareral][
            ...
            abi.encodeWithSelector(
                IStabilityPool.initialize.selector,
                _addressesRegistry
            )
        );
        assert(
    ...
    function _deployActivePool(
        ...
        branchContractAddresses[_collareral][
            ...
            abi.encodeWithSelector(
                IActivePool.initialize.selector,
                _addressesRegistry,
                SP_YIELD_SP
            )
        );
    ...
    function _deployDefaultPool(
        address _addressesRegistry,
        Collaterals _collareral
    ) internal {
        branchContractAddresses[_collareral][
            ContractTypes.DEFAULT_POOL
        ] = _deployProxy(
            contractImplementationAddresses[ContractTypes.DEFAULT_POOL],
            abi.encodeWithSelector(
                IDefaultPool.initialize.selector,
                _addressesRegistry
            )
        );
        assert(
    ...
    function _deployGasPool(
        address _addressesRegistry,
        Collaterals _collareral
    ) internal {
        branchContractAddresses[_collareral][
            ContractTypes.GAS_POOL
        ] = _deployProxy(
            contractImplementationAddresses[ContractTypes.GAS_POOL],
            abi.encodeWithSelector(
                GasPool.initialize.selector,
                _addressesRegistry
            )
        );
        assert(
            branchContractAddresses[_collareral][ContractTypes.GAS_POOL] ==
                _forecastAddressForGasPool(_collareral)
        );
    }

    function _deployCollSurplusPool(
        address _addressesRegistry,
        Collaterals _collareral
    ) internal {
        branchContractAddresses[_collareral][
            ...
            abi.encodeWithSelector(
                ICollSurplusPool.initialize.selector,
                _addressesRegistry
            )
        );
        assert(
    ...
    function _deploySortedTroves(
        address _addressesRegistry,
        Collaterals _collareral
    ) internal {
        branchContractAddresses[_collareral][
            ContractTypes.SORTED_TROVES
        ] = _deployProxy(
            contractImplementationAddresses[ContractTypes.SORTED_TROVES],
            abi.encodeWithSelector(
                ISortedTroves.initialize.selector,
                _addressesRegistry
            )
        );
        assert(
    ...
    function _forecastAddressForBorrowerOperations(
        Collaterals _collateral
    ) internal view returns (address _borrowerOperations) {
        address _addressesRegistry = branchContractAddresses[_collateral][
            ContractTypes.ADDRESSES_REGISTRY
        ];
    ...
    function _forecastAddressForTroveManager(
        Collaterals _collateral
    ) internal view returns (address _troveManager) {
        address _addressesRegistry = branchContractAddresses[_collateral][
            ContractTypes.ADDRESSES_REGISTRY
        ];
    ...
    function _forecastAddressForStabilityPool(
        Collaterals _collateral
    ) internal view returns (address _stabilityPool) {
        address _addressesRegistry = branchContractAddresses[_collateral][
            ContractTypes.ADDRESSES_REGISTRY
        ];
    ...
    function _forecastAddressForActivePool(
        Collaterals _collateral
    ) internal view returns (address _activePool) {
        address _addressesRegistry = branchContractAddresses[_collateral][
            ContractTypes.ADDRESSES_REGISTRY
        ];
    ...
    function _forecastAddressForDefaultPool(
        Collaterals _collateral
    ) internal view returns (address _defaultPool) {
        address _addressesRegistry = branchContractAddresses[_collateral][
            ContractTypes.ADDRESSES_REGISTRY
        ];
        _defaultPool = vm.computeCreate2Address(
            ...
            keccak256(
                _getBytecodeProxy(
                    contractImplementationAddresses[ContractTypes.DEFAULT_POOL],
                    abi.encodeWithSelector(
                        DefaultPool.initialize.selector,
                        _addressesRegistry
                    )
                )
            )
    ...
    function _forecastAddressForGasPool(
        Collaterals _collateral
    ) internal view returns (address _gasPool) {
        address _addressesRegistry = branchContractAddresses[_collateral][
            ContractTypes.ADDRESSES_REGISTRY
        ];
    ...
    function _forecastAddressForCollSurplusPool(
        Collaterals _collateral
    ) internal view returns (address _collSurplusPool) {
        address _addressesRegistry = branchContractAddresses[_collateral][
            ContractTypes.ADDRESSES_REGISTRY
        ];
        _collSurplusPool = vm.computeCreate2Address(
            ...
            keccak256(
                _getBytecodeProxy(
                    ...
                    abi.encodeWithSelector(
                        CollSurplusPool.initialize.selector,
                        _addressesRegistry
                    )
                )
            )
    ...
    function _forecastAddressForSortedTroves(
        Collaterals _collateral
    ) internal view returns (address _sortedTroves) {
        address _addressesRegistry = branchContractAddresses[_collateral][
            ContractTypes.ADDRESSES_REGISTRY
        ];
        _sortedTroves = vm.computeCreate2Address(
            ...
            keccak256(
                _getBytecodeProxy(
                    ...
                    abi.encodeWithSelector(
                        SortedTroves.initialize.selector,
                        _addressesRegistry
                    )
                )
            )
    ...
    function _forecastAddressForTroveNFT(
        Collaterals _collateral
    ) internal view returns (address _troveNFT) {
        address _addressesRegistry = branchContractAddresses[_collateral][
            ContractTypes.ADDRESSES_REGISTRY
        ];
    ...
    function _forecastAddressForCurveGaugeDistributor()
        internal
        view
        returns (address _curveGaugeDistributor)
    {
        _curveGaugeDistributor = vm.computeCreate2Address(
            SALT,
            keccak256(_getBytecodeCurveGaugeDistributor())
        );
    }

    function _transferOwnershipOfProxyAdminToAdminControllerAndProxyAdminForAdminControllerToMultiSigWallet()
        internal
    {
        ...
        address _proxyAdminForAdminController = proxyAdminAddresses
            .proxyAdminForAdminController;
        Ownable(_proxyAdminForAdminController).transferOwnership(
            multiSigWallet
        );
        assert(
            Ownable(_proxyAdminForAdminController).owner() == multiSigWallet
        );
    }

    function _transferOwnershipOfCollateralRegistryToAdminController()
        internal
    {
        address _adminController = singletonContractAddresses.adminController;
        address _collateralRegistry = singletonContractAddresses
            .collateralRegistry;
        OwnableUpgradeable(_collateralRegistry).transferOwnership(
            _adminController
        );
        assert(
            OwnableUpgradeable(_collateralRegistry).owner() == _adminController
        );
    }
    ...
    function _getBytecodeCurveGaugeDistributor()
        internal
        view
    ...
    function _performSanityChecksOnfeUSDToken() internal {
        ...
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            ...
            assert(
                _feUSDToken.borrowerOperationsAddresses(_borrowerOperations)
            );
    ...
    function _performSanityChecksOnCollateralRegistry() internal {
        ICollateralRegistry _collateralRegistry = ICollateralRegistry(
            singletonContractAddresses.collateralRegistry
        );
    ...
    function _checkCollateralTokensInCollateralRegistry(
    ...
    function _checkCollateralTroveManagersInCollateralRegistry(
        ICollateralRegistry _collateralRegistry
    ) internal {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            address _troveManagerDeployed = branchContractAddresses[
                _collateral
            ][ContractTypes.TROVE_MANAGER];
            address _troveManagerFromRegistry = address(
                _collateralRegistry.troveManagers(i)
            );
    ...
    function _checkfeUSDTokenInCollateralRegistry(
        ICollateralRegistry _collateralRegistry
    ) internal {
        address _feUSDTokenFromRegistry = address(
            _collateralRegistry.feUSDToken()
        );
    ...
    function _checkOwnerInCollateralRegistry(
        ICollateralRegistry _collateralRegistry
    ) internal {
        address _ownerFromRegistry = OwnableUpgradeable(
    ...
    function _performSanityChecksOnCriticalParamsForBorrowerOperations()
        internal
    {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            ...
            address _branchPriceFeed = branchContractAddresses[_collateral][
                ContractTypes.PRICE_FEEDS
            ];
            address _branchActivePool = branchContractAddresses[_collateral][
                ContractTypes.ACTIVE_POOL
            ];
            address _branchDefaultPool = branchContractAddresses[_collateral][
                ContractTypes.DEFAULT_POOL
            ];

            assert(
                _borrowerOperations.MCR() == collateralParams[_collateral].MCR
            );
            assert(
                _borrowerOperations.CCR() == collateralParams[_collateral].CCR
            );
            assert(
                _borrowerOperations.SCR() == collateralParams[_collateral].SCR
            );
            assert(
                _borrowerOperations.maxDebtCap() ==
                    collateralParams[_collateral].maxDebtCap
            );
    ...
    function _performSanityChecksOnActivePool() internal {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            IActivePool _activePool = IActivePool(
                branchContractAddresses[_collateral][ContractTypes.ACTIVE_POOL]
            );
            assert(
                address(_activePool.collToken()) ==
                    address(collateralParams[_collateral].collToken)
            );
            assert(
            ...
            assert(
                address(_activePool.feUSDToken()) ==
                    singletonContractAddresses.feUSDToken
            );
            assert(
    ...
    function _performSanityChecksOnCollSurplusPool() internal {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            ICollSurplusPool _collSurplusPool = ICollSurplusPool(
                branchContractAddresses[_collateral][
                    ContractTypes.COLL_SURPLUS_POOL
                ]
            );
            assert(
                address(_collSurplusPool.collToken()) ==
                    address(collateralParams[_collateral].collToken)
            );
            assert(
    ...
    function _performSanityChecksOnDefaultPool() internal {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            IDefaultPool _defaultPool = IDefaultPool(
                branchContractAddresses[_collateral][ContractTypes.DEFAULT_POOL]
            );
            assert(
                address(_defaultPool.collToken()) ==
                    address(collateralParams[_collateral].collToken)
            );
            assert(
    ...
    function _performSanityChecksOnGasPool() internal {
        ...
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            GasPool _gasPool = GasPool(
                branchContractAddresses[_collateral][ContractTypes.GAS_POOL]
            );
            IWHYPE _whype = IWHYPE(
                collateralParams[Collaterals.WHYPE].collToken
            );
            assert(
                _whype.allowance(
    ...
    function _performSanityChecksOnHintHelpers() internal {
        IHintHelpers _hintHelpers = IHintHelpers(
            singletonContractAddresses.hintHelpers
        );
        assert(
            address(_hintHelpers.collateralRegistry()) ==
                singletonContractAddresses.collateralRegistry
        );
    }

    function _performSanityChecksOnMultiTroveGetter() internal {
        IMultiTroveGetter _multiTroveGetter = IMultiTroveGetter(
            singletonContractAddresses.multiTroveGetter
        );
        assert(
            address(_multiTroveGetter.collateralRegistry()) ==
                singletonContractAddresses.collateralRegistry
        );
    }
    ...
    function _performSanityChecksOnSortedTroves() internal {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            ISortedTroves _sortedTroves = ISortedTroves(
                branchContractAddresses[_collateral][
                    ContractTypes.SORTED_TROVES
                ]
            );
    ...
    function _performSanityChecksOnStabilityPool() internal {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            StabilityPool _stabilityPool = StabilityPool(
                branchContractAddresses[_collateral][
                    ContractTypes.STABILITY_POOL
                ]
            );
            ...
            (
                address _stabilityPoolActivePool,
                address _stabilityPoolDefaultPool,
                address _stabilityPoolPriceFeed
            ) = _getStorageValuesForLiquityBase(address(_stabilityPool));

            address _branchTroveManager = branchContractAddresses[_collateral][
                ContractTypes.TROVE_MANAGER
            ];
            ...
            assert(
                address(_stabilityPool.collToken()) ==
                    address(collateralParams[_collateral].collToken)
            );
            assert(
                address(_stabilityPool.troveManager()) == _branchTroveManager
            );
    ...
    function _performSanityChecksOnTroveManager() internal {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            TroveManager _troveManager = TroveManager(
                branchContractAddresses[_collateral][
                    ContractTypes.TROVE_MANAGER
                ]
            );
            ...
            (
                address _troveManagerActivePool,
                address _troveManagerDefaultPool,
                address _troveManagerPriceFeed
            ...
            assert(
                _troveManagerActivePool ==
                    branchContractAddresses[_collateral][
                        ContractTypes.ACTIVE_POOL
                    ]
            );
            assert(
                _troveManagerDefaultPool ==
                    branchContractAddresses[_collateral][
                        ContractTypes.DEFAULT_POOL
                    ]
            );
            assert(
                _troveManagerPriceFeed ==
                    branchContractAddresses[_collateral][
                        ContractTypes.PRICE_FEEDS
                    ]
            );
            assert(
                _whype == address(collateralParams[Collaterals.WHYPE].collToken)
            );
            ...
            assert(
                address(_troveManager.troveNFT()) ==
                    branchContractAddresses[_collateral][
                        ContractTypes.TROVE_NFT
                    ]
            );
            ...
            assert(
                _liquidationPenaltySp ==
                    collateralParams[_collateral].LIQUIDATION_PENALTY_SP
            );
            assert(
    ...
    function _performSanityChecksOnTroveNFT() internal {
        for (uint8 i = 0; i < COLLATERALS_LENGTH; i++) {
            Collaterals _collateral = Collaterals(i);
            TroveNFT _troveNFT = TroveNFT(
                branchContractAddresses[_collateral][ContractTypes.TROVE_NFT]
            );
            assert(
                address(_troveNFT.collToken()) ==
                    address(collateralParams[_collateral].collToken)
            );
            assert(
                address(_troveNFT.feUSDToken()) ==
                    singletonContractAddresses.feUSDToken
            );
            assert(
                address(_troveNFT.metadataNFT()) ==
                    singletonContractAddresses.metadataNFT
            );
            assert(
                address(_troveNFT.troveManager()) ==
                    branchContractAddresses[_collateral][
                        ContractTypes.TROVE_MANAGER
                    ]
            );
    ...
    function _getStorageValuesForTroveManager(
        ...
        )
    {
        _gasPool = address(
            uint160(
                uint256(
                    vm.load(
                        address(_troveManager),
                        bytes32(uint256(GAS_POOL_SLOT_FOR_TROVE_MANAGER))
                    )
                )
            )
        );
            ...
            uint160(
                uint256(
                    vm.load(
                        address(_troveManager),
                        bytes32(
                            uint256(COLL_SURPLUS_POOL_SLOT_FOR_TROVE_MANAGER)
                        )
                    )
                )
            )
        ...
        _feUSDToken = address(
            uint160(
                uint256(
                    vm.load(
                        address(_troveManager),
                        bytes32(uint256(FEUSD_TOKEN_SLOT_FOR_TROVE_MANAGER))
                    )
                )
            )
        );
        _whype = address(
            uint160(
                uint256(
                    vm.load(
                        address(_troveManager),
                        bytes32(uint256(WHYPE_SLOT_FOR_TROVE_MANAGER))
                    )
                )
            )
        );
        _mcr = uint256(
            vm.load(
                address(_troveManager),
                bytes32(uint256(MCR_SLOT_FOR_TROVE_MANAGER))
            )
        );
        _scr = uint256(
            vm.load(
                address(_troveManager),
                bytes32(uint256(SCR_SLOT_FOR_TROVE_MANAGER))
            )
        );
        _liquidationPenaltySp = uint256(
            vm.load(
                address(_troveManager),
                bytes32(uint256(LIQUIDATION_PENALTY_SP_SLOT_FOR_TROVE_MANAGER))
            )
        );
        _liquidationPenaltyRedistribution = uint256(
            vm.load(
                ...
                bytes32(
                    uint256(
                        LIQUIDATION_PENALTY_REDISTRIBUTION_SLOT_FOR_TROVE_MANAGER
                    )
                )
            )
        );
    ...
    function _getStorageValuesForLiquityBase(
        address _contract
    )
        internal
        view
        ...
        _activePool = address(
            uint160(
                uint256(
                    vm.load(
                        address(_contract),
                        bytes32(uint256(ACTIVE_POOL_SLOT_FOR_LQ_BASE))
        ...
        _defaultPool = address(
            uint160(
                uint256(
                    vm.load(
                        address(_contract),
                        bytes32(uint256(DEFAULT_POOL_SLOT_FOR_LQ_BASE))
                    )
                )
            )
        );
        _priceFeed = address(
            uint160(
                uint256(
                    vm.load(
                        address(_contract),
                        bytes32(uint256(PRICE_FEED_SLOT_FOR_LQ_BASE))
                    )
                )
            )
        );
                ...
                string.concat(
                    ...
                    string.concat(
                        ...
                        string.concat(
                            string.concat(
                                '"name":"',
                                _getNameForCollateral(_collateral),
                                '",'
                            ),
                            string.concat(
                                '"addressesRegistry":"',
    ...
    function _getNameForCollateral(
        Collaterals _collateral
            ...
            string.concat(
                ...
                string.concat(
                    string.concat(
                        ...
                        '",',
                        '"feUSDToken":"',
                        singletonContractAddresses.feUSDToken.toHexString(),
                        '",',
                        '"hintHelpers":"',
                        singletonContractAddresses.hintHelpers.toHexString(),
                        '",',
                        '"multiTroveGetter":"',
                        singletonContractAddresses
                            .multiTroveGetter
                        ...
                        '",',
                        '"metadataNFT":"',
                        singletonContractAddresses.metadataNFT.toHexString(),
                        '",',
                        '"curvePool":"',
                        curveAddresses.curvePool.toHexString(),
                        '",',
                        '"curveGauge":"',
                        curveAddresses.curveGauge.toHexString(),
                        '",',
                        '"curveGaugeDistributor":"',
                        curveAddresses.curveGaugeDistributor.toHexString(),
                        '",'
                    ),
                    '"branches":[',
                    branches.join(","),
    ...
    function _createInterestRouterAllocationConfig()
        internal
        view
        returns (IInterestRouterV2.AllocationConfig memory)
    {
        IInterestRouterV2.AllocationConfig
            memory _allocationConfig = IInterestRouterV2.AllocationConfig({
    ...
    function _createInterestRouterDestinationsArray()
        internal
        view
        returns (address[] memory)
    {
        address[] memory _destinations = new address[](
            REWARD_DESTINATIONS_LENGTH
        );
        _destinations[0] = _forecastAddressForCurveGaugeDistributor();
        return _destinations;
    }

    function _createInterestRouterPercentagesArray()
        internal
        view
        returns (uint256[] memory)
    {
        uint256[] memory _percentages = new uint256[](
            REWARD_DESTINATIONS_LENGTH
        );
    ...
    function _createInterestRouterSelectorsArray()
        internal
        view
    ...
    function _deployCurveGaugeDistributor() internal {
        ...
        address _gaugeDistributor = address(
            new CurveGaugeDistributor{salt: SALT}(
                _feUSDToken,
                _tempOwner
            )
        );
    ...
    function _setCurveGaugeDistributorInterestRouter() internal {
        ...
        address _interestRouter = branchContractAddresses[Collaterals.WHYPE][
            ContractTypes.INTEREST_ROUTER
        ];
        assert(_gaugeDistributor != address(0));
        assert(_interestRouter != address(0));
        CurveGaugeDistributor(_gaugeDistributor).setInterestRouterAndCurveGauge(
            _interestRouter,
            curveAddresses.curveGauge
        );
    ...
    function _deployCurvePool() internal {
        address _pool = CURVE_CRYPTO_SWAP_FACTORY.deploy_pool(
            POOL_NAME,
            POOL_SYMBOL,
            [WHYPE_ADDRESS, singletonContractAddresses.feUSDToken],
            IMPLEMENTATION_ID,
            A,
            GAMMA,
            MID_FEE,
        ...
        curveAddresses.curvePool = _pool;
    }

    function _setCurveAddresses() internal {
        address _pool = curveAddresses.curvePool;

        address _coin0 = ICurveStableswapNGPool(_pool).coins(0);
        address _coin1 = ICurveStableswapNGPool(_pool).coins(1);

        assert(
            _coin0 == singletonContractAddresses.feUSDToken ||
                _coin0 == WHYPE_ADDRESS
        );
        assert(
            _coin1 == singletonContractAddresses.feUSDToken ||
                _coin1 == WHYPE_ADDRESS
        );
    ...
    function _deployCurveGauge() internal {
        address _gauge = CURVE_X_CHAIN_LIQUIDITY_GAUGE_FACTORY.deploy_gauge(
            curveAddresses.curvePool,
            GAUGE_SALT,
            deployerInfo.deployerAddress
        );
    ...
    function _getCoinsArrayCurvePool()
        internal
        view
        returns (address[2] memory _coins)
    {
        _coins[0] = singletonContractAddresses.feUSDToken;
        _coins[1] = WHYPE_ADDRESS;
        return _coins;
    }
    ...
    function _loadTimestampFixture() internal {
        ...
        (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound) = AggregatorV3Interface(_collateralParams.redStonePriceFeedAddress).latestRoundData();
